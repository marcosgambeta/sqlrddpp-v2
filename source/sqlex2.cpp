...
void CreateInsertStmt(SQLEXAREAP thiswa)
{
  ...
  for (i = 1; i <= iCols; i++)
  {
    ...
    if (i != static_cast<int>(thiswa->ulhRecno))
    { // RECNO is never included in INSERT column list
      temp = hb_strdup(static_cast<const char *>(sFields));
      sprintf(sFields, "%s,%c%s%c", temp, OPEN_QUALIFIER(thiswa), QualifyName(colName, thiswa),
...
HB_ERRCODE BindInsertColumns(SQLEXAREAP thiswa)
{
  ...
  {
    if (iCol != static_cast<int>(thiswa->ulhRecno))
    { // RECNO is never included in INSERT column list
      iBind++;
      ...
      case SQL_C_CHAR: {
        ...
        res = SQLBindParameter(thiswa->hStmtInsert, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
                               static_cast<SQLSMALLINT>(InsertRecord->iCType),
                               static_cast<SQLSMALLINT>(InsertRecord->iSQLType), InsertRecord->ColumnSize,
                               InsertRecord->DecimalDigits, InsertRecord->asChar.value, 0, &(InsertRecord->lIndPtr));
        break;
      }
      case SQL_C_BINARY: {
        SQLINTEGER nInd;
      ...
      case SQL_C_DOUBLE: {
        ...
        res = SQLBindParameter(thiswa->hStmtInsert, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
                               static_cast<SQLSMALLINT>(InsertRecord->iCType),
                               static_cast<SQLSMALLINT>(InsertRecord->iSQLType), InsertRecord->ColumnSize,
                               InsertRecord->DecimalDigits, &(InsertRecord->asNumeric), 0, &(InsertRecord->lIndPtr));
        break;
      }
      ...
      case SQL_C_TYPE_DATE: {
        InsertRecord->lIndPtr = 0;
        res = SQLBindParameter(thiswa->hStmtInsert, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT, SQL_C_TYPE_DATE,
                               SQL_TYPE_DATE, SQL_DATE_LEN, 0, &(InsertRecord->asDate), 0, &(InsertRecord->lIndPtr));
        break;
      }
      case SQL_C_BIT: {
        res = SQLBindParameter(thiswa->hStmtInsert, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
                               static_cast<SQLSMALLINT>(InsertRecord->iCType),
                               static_cast<SQLSMALLINT>(InsertRecord->iSQLType), InsertRecord->ColumnSize,
                               InsertRecord->DecimalDigits, &(InsertRecord->asLogical), 0, nullptr);
        break;
      }
      }
...
HB_ERRCODE FeedRecordCols(SQLEXAREAP thiswa, HB_BOOL bUpdate)
{
  ...
  for (i = 1; i <= iCols; i++)
  {
    if ((!bUpdate) || (bUpdate && (thiswa->editMask[i - 1] || thiswa->specialMask[i - 1])))
    {
      ...
      else if (i != static_cast<int>(thiswa->ulhRecno))
      { // RECNO is never included in INSERT column list
...
HB_ERRCODE CreateUpdateStmt(SQLEXAREAP thiswa)
{
  ...
  for (i = 0; i < iCols; i++)
  {
    if (thiswa->editMask[i] || thiswa->specialMask[i])
    {
      ...
      case SQL_C_CHAR: {
        ...
        res = SQLBindParameter(thiswa->hStmtUpdate, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
                               static_cast<SQLSMALLINT>(CurrRecord->iCType),
                               static_cast<SQLSMALLINT>(CurrRecord->iSQLType), CurrRecord->ColumnSize,
                               CurrRecord->DecimalDigits, CurrRecord->asChar.value, 0, &(CurrRecord->lIndPtr));
        break;
      }
      case SQL_C_BINARY: {
        SQLINTEGER nInd;
      ...
      case SQL_C_DOUBLE: {
        ...
        res = SQLBindParameter(thiswa->hStmtUpdate, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
                               static_cast<SQLSMALLINT>(CurrRecord->iCType),
                               static_cast<SQLSMALLINT>(CurrRecord->iSQLType), CurrRecord->ColumnSize,
                               CurrRecord->DecimalDigits, &(CurrRecord->asNumeric), 0, &(CurrRecord->lIndPtr));
        break;
      }
      case SQL_C_TYPE_TIMESTAMP: {
        // DebugBreak();
      ...
      case SQL_C_TYPE_DATE: {
        CurrRecord->lIndPtr = 0;
        res = SQLBindParameter(thiswa->hStmtUpdate, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT, SQL_C_TYPE_DATE,
                               SQL_TYPE_DATE, SQL_DATE_LEN, 0, &(CurrRecord->asDate), 0, &(CurrRecord->lIndPtr));
        break;
      }
      case SQL_C_BIT: {
        res = SQLBindParameter(thiswa->hStmtUpdate, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
                               static_cast<SQLSMALLINT>(CurrRecord->iCType),
                               static_cast<SQLSMALLINT>(CurrRecord->iSQLType), CurrRecord->ColumnSize,
                               CurrRecord->DecimalDigits, &(CurrRecord->asLogical), 0, nullptr);
        break;
      }
      }
      ...
      sprintf(thiswa->sSql, "%s%c %c%s%c = ?", temp, iBind > 1 ? ',' : ' ', OPEN_QUALIFIER(thiswa), CurrRecord->colName,
              CLOSE_QUALIFIER(thiswa));
      hb_xfree(temp);
  ...
  sprintf(thiswa->sSql, "%s\n WHERE %c%s%c = ?", temp, OPEN_QUALIFIER(thiswa), thiswa->sRecnoName,
          CLOSE_QUALIFIER(thiswa));
  hb_xfree(temp);
  res = SQLBindParameter(thiswa->hStmtUpdate, static_cast<SQLUSMALLINT>(++iBind), SQL_PARAM_INPUT, SQL_C_ULONG,
                         SQL_INTEGER, 15, 0, &(thiswa->lUpdatedRecord), 0, nullptr);
  if (CHECK_SQL_N_OK(res))
  {
    odbcErrorDiagRTE(thiswa->hStmtUpdate, "BindUpdateColumns", thiswa->sSql, res, __LINE__, __FILE__);
    return Harbour::FAILURE;
...
HB_ERRCODE ExecuteUpdateStmt(SQLEXAREAP thiswa)
{
  ...
  if (FeedRecordCols(thiswa, true) == Harbour::FAILURE)
