      ...
      IF ::lCanUpd
         FOR nThisField := 1 TO ::nFields
            ...
            IF lML .OR. ;
               ...
               ENDIF
               IF (lMemo .OR. lML) .AND. ::aFields[nThisField, SR_FIELD_DOMAIN] != SQL_FAKE_LOB  
...
METHOD SR_WORKAREA:sqlSeek(uKey, lSoft, lLast)
   ...
   IF lSoft .AND. ::lISAM .AND. ::aIndex[::aInfo[SR_AINFO_INDEXORD], VIRTUAL_INDEX_NAME] != NIL .AND. HB_IsChar(uKey)
...
METHOD SR_WORKAREA:sqlCreate(aStruct, cFileName, cAlias, nArea)
   ...
   IF SR_UseDeleteds()
      AAdd(aCreate, {::cDeletedName, "C", 1, 0, .T., , MULTILANG_FIELD_OFF, , , 0, .F.})
   ENDIF
   ...
   IF !Empty(SR_SetTblSpaceData())
      cSql += " TABLESPACE " + SR_SetTblSpaceData()
   ENDIF

   IF (!Empty(SR_SetTblSpaceLob())) .AND. (!Empty(cLobs))
   ...
   IF lRet .AND. ::oSql:lUseSequences // Create RECNO Trigger
      ...
      FOR i := 1 TO Len(aCols)
         ...
         IF (nPos := AScan(::aNames, {|x|x == cCol})) != 0
            IF ::aNames[nPos] != ::cRecnoName
               ...
               SWITCH ::aFields[nPos, SR_FIELD_TYPE]
               CASE "C"
                  IF ::aNames[nPos] == ::cDeletedName // TODO: check this code
                     cXBase += "Deleted() + "
                  ELSE
                     cXBase += ::aNames[nPos] + " + "
                  ENDIF
...
METHOD SR_WORKAREA:sqlOrderCreate(cIndexName, cColumns, cTag, cConstraintName, cTargetTable, aTargetColumns, lEnable)
   ...
   IF !lSyntheticIndex .AND. Len(aCols) > SR_GetSyntheticIndexMinimun()
      // Oracle can workaround with SinthetycVirtualIndex
   ...
   ENDIF
   IF Len(cPhisicalName) > 30     // Oracle sucks!
...
METHOD SR_WORKAREA:CreateConstraint(cSourceTable, aSourceColumns, cTargetTable, aTargetColumns, cConstraintName)
   ...
   IF Len(aRet) == 0
      ...
      IF Len(cConstraintName) > 30     // Oracle sucks!
