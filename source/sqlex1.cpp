...
static bool CreateSkipStmt(SQLEXAREA *thiswa);
static int s_bOldReverseIndex = 0;
static int sqlKeyCompareEx(SQLEXAREA *thiswa, PHB_ITEM pKey, HB_BOOL fExact);
static PHB_DYNS s_pSym_SR_DESERIALIZE = nullptr;
// static HB_BOOL _SqlExIsLogFirst = true; not used
static HB_BOOL s__SqlExIsLogFile = false;

HB_BOOL SqlExIsLog();
void SqlExLog(const char *str, int ver);

HB_EXTERN_BEGIN
extern PHB_ITEM sr_loadTagDefault(SQLEXAREA *thiswa, LPDBORDERINFO pInfo, HB_LONG *lorder);
HB_EXTERN_END

//------------------------------------------------------------------------

#if 0
static char *sqlSolveRestrictors(SQLEXAREA *thiswa)
{
...
static HB_ERRCODE ConcludeSkipraw(SQLEXAREA *thiswa)
{
...
static void sqlGetCleanBuffer(SQLEXAREA *thiswa)
{
  HB_SIZE nPos, nLen;
...
static void createRecodListQuery(SQLEXAREA *thiswa)
{
  if (thiswa->sSql) {
  ...
  if (thiswa->ulhDeleted == 0) {
    if (thiswa->bIsSelect) {
      sprintf(thiswa->sSql, "SELECT %s A.%c%s%c FROM (%s) A %s %s %s", thiswa->sLimit1,
              ...
              thiswa->szDataFileName, thiswa->sWhere, thiswa->sOrderBy, thiswa->sLimit2);
    } else {
      sprintf(thiswa->sSql, "SELECT %s A.%c%s%c FROM %s A %s %s %s", thiswa->sLimit1,
              OPEN_QUALIFIER(thiswa), thiswa->sRecnoName.c_str(), CLOSE_QUALIFIER(thiswa),
...
static void createCountQuery(SQLEXAREA *thiswa)
{
  if (thiswa->sSql) {
    memset(thiswa->sSql, 0, MAX_SQL_QUERY_LEN * sizeof(char));
  }
...
static HB_ERRCODE getMissingColumn(SQLEXAREA *thiswa, PHB_ITEM pFieldData, HB_LONG lFieldPosDB)
{
  char *colName;
  ...
  if (thiswa->colStmt[lFieldPosDB - 1] == nullptr) {
    ...
    if (thiswa->bIsSelect) {
      sprintf(sSql, "SELECT %c%s%c FROM (%s) WHERE %c%s%c = ?", OPEN_QUALIFIER(thiswa), colName,
              CLOSE_QUALIFIER(thiswa), thiswa->szDataFileName, OPEN_QUALIFIER(thiswa),
              thiswa->sRecnoName.c_str(), CLOSE_QUALIFIER(thiswa));
    } else {
      sprintf(sSql, "SELECT %c%s%c FROM %s WHERE %c%s%c = ?", OPEN_QUALIFIER(thiswa), colName,
              CLOSE_QUALIFIER(thiswa), thiswa->sTable.c_str(), OPEN_QUALIFIER(thiswa),
              thiswa->sRecnoName.c_str(), CLOSE_QUALIFIER(thiswa));
    }
    hb_xfree(colName);

    res = SQLPrepare(thiswa->colStmt[lFieldPosDB - 1], reinterpret_cast<SQLCHAR *>(sSql),
                     SQL_NTS);
    ...
    res = SQLBindParameter(thiswa->colStmt[lFieldPosDB - 1], 1, SQL_PARAM_INPUT, SQL_C_ULONG,
                           SQL_INTEGER, 15, 0, &(thiswa->lCurrentRecord), 0, nullptr);
  ...
  thiswa->lCurrentRecord = SR_GetCurrentRecordNum(thiswa); // Feed bound parameter

  res = SQLExecute(thiswa->colStmt[lFieldPosDB - 1]);

  if (CHECK_SQL_N_OK(res)) {
    SQLRDD::odbcErrorDiagRTE(thiswa->colStmt[lFieldPosDB - 1], "getMissingColumn/SQLExecute",
                             sSql, (SQLRETURN)res, __LINE__, __FILE__);
  ...
  if (res != SQL_SUCCESS) {
    if (res == SQL_ERROR) {
      SQLRDD::odbcErrorDiagRTE(thiswa->colStmt[lFieldPosDB - 1], "getMissingColumn/SQLFetch",
                               sSql, (SQLRETURN)res, __LINE__, __FILE__);
      SQLFreeStmt(thiswa->colStmt[lFieldPosDB - 1], SQL_CLOSE);
      return HB_FAILURE;
    }
  ...
  case SQL_C_BINARY: {
    ...
    if (nTrim >= BindStructure->asChar.size_alloc) {
      BindStructure->asChar.value =
          static_cast<SQLCHAR *>(hb_xrealloc(BindStructure->asChar.value, nTrim + 1));
    ...
    if (nTrim >= BindStructure->asChar.size_alloc || nTrim > BindStructure->asChar.size) {
      ...
      res = SQLBindParameter(hStmt, static_cast<SQLUSMALLINT>(BindStructure->iParNum),
                             SQL_PARAM_INPUT, SQL_C_CHAR, SQL_LONGVARCHAR,
                             BindStructure->asChar.size_alloc, 0, BindStructure->asChar.value,
                             0, &(BindStructure->lIndPtr));
...
#ifdef __XHARBOUR__
      double seconds;
      hb_datetimeDecode(pFieldData->item.asDate.value, pFieldData->item.asDate.time, &iYear,
...
void ReleaseColStatements(SQLEXAREA *thiswa, int iCols)
{
  int i;
  if (thiswa->colStmt) {
    if (iCols == 0) {
...
{
  thiswa->colStmt =
      static_cast<HSTMT *>(hb_xgrab(hb_arrayLen(thiswa->aFields) * sizeof(HSTMT)));
...
static void BindAllIndexStmts(SQLEXAREA *thiswa)
{
  ...
  if (thiswa->hOrdCurrent == 0) {
    ...
    res = SQLBindParameter(
        hStmt, 1, SQL_PARAM_INPUT, static_cast<SQLSMALLINT>(BindStructure->iCType),
        ...
        if (!BindStructure->isArgumentNull) {
          ...
          case SQL_C_CHAR: {
            res = SQLBindParameter(hStmt, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
                                   ...
                                   BindStructure->ColumnSize, BindStructure->DecimalDigits,
                                   BindStructure->asChar.value, 0, nullptr);
            break;
          }
          case SQL_C_DOUBLE: {
            res = SQLBindParameter(hStmt, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
          ...
          case SQL_C_TYPE_TIMESTAMP: {
            ...
            //                        0);
            res = SQLBindParameter(hStmt, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
                                   SQL_C_TYPE_TIMESTAMP, SQL_TYPE_TIMESTAMP, SQL_TIMESTAMP_LEN,
                                   thiswa->nSystemID == SQLRDD::RDBMS::MSSQL7 ||
                                           thiswa->nSystemID == SQLRDD::RDBMS::AZURE
                                       ? 3
                                       : 0,
          ...
          case SQL_C_BIT: {
            res = SQLBindParameter(hStmt, static_cast<SQLUSMALLINT>(iBind), SQL_PARAM_INPUT,
                                   ...
                                   static_cast<SQLSMALLINT>(BindStructure->iSQLType),
                                   BindStructure->ColumnSize, BindStructure->DecimalDigits,
                                   &(BindStructure->asLogical), 0, nullptr);
          }
          }
          if (CHECK_SQL_N_OK(res)) {
            SQLRDD::odbcErrorDiagRTE(hStmt, "BindAllIndexStmts()", sSql, res, __LINE__,
                                     __FILE__);
          }
          iBind++;
        }
...
static void FeedCurrentRecordToBindings(SQLEXAREA *thiswa)
{
  PHB_ITEM pFieldData;
  int iCol;
  INDEXBIND *IndexBind;
    ...
    for (iCol = 1; iCol <= thiswa->indexColumns; iCol++) {
      ...
      if (SQLRDD::itemEmpty(pFieldData) &&
        ...
        if (BindStructure->isNullable && BindStructure->isArgumentNull) {
          ...
          SR_SetBindEmptylValue(BindStructure);
...
void SR_SetIndexBindStructure(SQLEXAREA *thiswa)
{
  ...
  if (thiswa->hOrdCurrent > 0) {
    ...
    thiswa->IndexBindings[thiswa->hOrdCurrent] =
        static_cast<INDEXBIND *>(hb_xgrab(thiswa->indexColumns * sizeof(INDEXBIND)));
    memset(thiswa->IndexBindings[thiswa->hOrdCurrent], 0,
           thiswa->indexColumns * sizeof(INDEXBIND));
...
void SR_SetCurrRecordStructure(SQLEXAREA *thiswa)
{
  ...
  for (i = 1; i <= iCols; i++) {
    ...
    case 'C': {
      BindStructure->asChar.value =
          static_cast<SQLCHAR *>(hb_xgrabz(BindStructure->ColumnSize + 1));
...
static HB_ERRCODE getWhereExpression(SQLEXAREA *thiswa, int iListType)
{
  ...
  if (iListType == SR_LIST_SKIP_FWD || iListType == SR_LIST_SKIP_BWD) {
    ...
    if (thiswa->hOrdCurrent == 0) { // Natural order
      sprintf(thiswa->sWhere, "\nWHERE A.%c%s%c %s ?", OPEN_QUALIFIER(thiswa),
              thiswa->sRecnoName.c_str(), CLOSE_QUALIFIER(thiswa), bDirectionFWD ? ">=" : "<=");
      BindStructure = SR_GetBindStruct(thiswa, IndexBind);
      BindStructure->asNumeric = static_cast<SQLDOUBLE>(SR_GetCurrentRecordNum(thiswa));
      bWhere = true;
    } else {
...
HB_ERRCODE getWorkareaParams(SQLEXAREA *thiswa)
{
  HB_ULONG lCnnType;
  ...
  if (!thiswa->oSql) {
    ...
    thiswa->sRecnoName =
        SQLRDD::QualifyName(const_cast<char *>(thiswa->sRecnoName.c_str()), thiswa);
    thiswa->sDeletedName =
        SQLRDD::QualifyName(const_cast<char *>(thiswa->sDeletedName.c_str()), thiswa);
  ...
  if (!thiswa->bConnVerified) {
    ...
    if (!(lCnnType == CONNECT_ODBC || lCnnType == CONNECT_ODBC_QUERY_ONLY)) {
      SQLRDD::commonError(reinterpret_cast<AREAP>(thiswa), EG_OPEN, ESQLRDD_OPEN,
                          "SQLEX supports only ODBC connections.");
      return HB_FAILURE;
    }
...
static HB_ERRCODE getPreparedRecordList(SQLEXAREA *thiswa,
                                        int iMax) // Returns true if any result found
{
  ...
  for (i = 0; i < iMax; i++) {
    ...
    if (res != SQL_SUCCESS) {
      ...
      } else if (i == 0 && thiswa->indexLevel == 1 && res == SQL_NO_DATA_FOUND &&
    ...
    res = SQLGetData(hStmt, 1, SQL_C_ULONG, &(thiswa->recordList[i]), sizeof(SQL_C_ULONG),
                     nullptr);
    if (res == SQL_ERROR) {
      SQLFreeStmt(hStmt, SQL_CLOSE);
      return HB_FAILURE;
...
static HB_ERRCODE getRecordList(SQLEXAREA *thiswa, int iMax) // Returns true if any result found
{
  SQLRETURN res;
  int i, recordListChanged;
...
static HB_ERRCODE getFirstColumnAsLong(SQLEXAREA *thiswa,
                                       long *szValue) // Returns OK if result set could be get
{
  SQLRETURN res;
...
static HB_ERRCODE updateRecordBuffer(SQLEXAREA *thiswa, HB_BOOL bUpdateDeleted)
{
  SQLRETURN res;
  ...
  if (SR_getColumnList(thiswa) ||
    ...
    if (thiswa->bIsSelect) {
      sprintf(thiswa->sSqlBuffer, "SELECT %s FROM (%s) A WHERE A.%c%s%c IN ( ?",
              thiswa->sFields, thiswa->szDataFileName, OPEN_QUALIFIER(thiswa),
              thiswa->sRecnoName.c_str(), CLOSE_QUALIFIER(thiswa));
    } else {
    ...
    res = SQLPrepare(thiswa->hStmtBuffer, reinterpret_cast<SQLCHAR *>(thiswa->sSqlBuffer),
                     SQL_NTS);
    if (CHECK_SQL_N_OK(res)) {
      return HB_FAILURE;
    }
    ...
    for (i = 0; i < s_pageReadSize; i++) {
      res = SQLBindParameter(thiswa->hStmtBuffer, i + 1, SQL_PARAM_INPUT, SQL_C_ULONG,
                             SQL_INTEGER, 15, 0, &(thiswa->lRecordToRetrieve[i]), 0, nullptr);
  ...
  if (CHECK_SQL_N_OK(res)) {
    SQLRDD::odbcErrorDiagRTE(thiswa->hStmtBuffer, "updateRecordBuffer", thiswa->sSqlBuffer, res,
                             __LINE__, __FILE__);
  ...
  for (iRow = 1; iRow <= s_pageReadSize; iRow++) {
    ...
    res = SQLGetData(thiswa->hStmtBuffer, 1, SQL_C_ULONG, &(lCurrRecord), sizeof(SQL_C_ULONG),
                     nullptr);
    if (res == SQL_ERROR) {
...
static HB_ERRCODE trySkippingOnCache(SQLEXAREA *thiswa, HB_LONG lToSkip)
{
    ...
    if (thiswa->recordListDirection == SR_LIST_FORWARD) {
      ...
      if (lSupposedPos >= 0 && lSupposedPos < thiswa->recordListSize) {
        ...
        if (updateRecordBuffer(thiswa, false) == HB_FAILURE) {
          SQLRDD::commonError(reinterpret_cast<AREAP>(thiswa), EG_ARG, ESQLRDD_READ,
                              thiswa->sTable.c_str());
          return HB_FAILURE;
        }
        return HB_SUCCESS;
...
static bool CreateSkipStmt(SQLEXAREA *thiswa)
{
  ...
  // thiswa->IndexBindings[thiswa->hOrdCurrent]))->index???Stmt only.
  ...
  if (thiswa->bOrderChanged || thiswa->bConditionChanged1 ||
    ...
    SR_setResultSetLimit(thiswa, RECORD_LIST_SIZE);
    ...
    for (int i = 1; i <= thiswa->indexColumns; i++) {
      getWhereExpression(thiswa, thiswa->recordListDirection == SR_LIST_FORWARD ? SR_LIST_SKIP_FWD
                                                                             : SR_LIST_SKIP_BWD);
...
// (DBENTRYP_BP)
static HB_ERRCODE sqlExBof(SQLEXAREA *thiswa, HB_BOOL *bof)
{
...
// (DBENTRYP_BP)
static HB_ERRCODE sqlExEof(SQLEXAREA *thiswa, HB_BOOL *eof)
{
  if (thiswa->firstinteract) {
    SELF_GOTOP(reinterpret_cast<AREAP>(thiswa));
    thiswa->firstinteract = false;
...
// (DBENTRYP_BP)
static HB_ERRCODE sqlExFound(SQLEXAREA *thiswa, HB_BOOL *found)
{
  if (thiswa->lpdbPendingRel) {
    SELF_FORCEREL(reinterpret_cast<AREAP>(thiswa));
  }
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExGoBottom(SQLEXAREA *thiswa)
{
  ...
  if (thiswa->lEofAt) {
    ...
    if (thiswa->bReverseIndex != s_bOldReverseIndex) {
      thiswa->recordListDirection = SR_LIST_BACKWARD;
      SR_getOrderByExpression(thiswa, false);
      getWhereExpression(thiswa, SR_LIST_FROM_BOTTOM);
      SR_setResultSetLimit(thiswa, RECORD_LIST_SIZE / 10);
      createRecodListQuery(thiswa);

      if (getRecordList(thiswa, RECORD_LIST_SIZE / 10) == HB_FAILURE) {
        SQLRDD::odbcErrorDiagRTE(thiswa->hStmt, "dbGoBottom", thiswa->sSql, SQL_ERROR, __LINE__,
                                 __FILE__);
        SQLRDD::commonError(reinterpret_cast<AREAP>(thiswa), EG_ARG, ESQLRDD_READ,
                            thiswa->sTable.c_str());
        return HB_FAILURE;
...
// (DBENTRYP_UL)
static HB_ERRCODE sqlExGoTo(SQLEXAREA *thiswa, HB_LONG recno)
{
  int i;
...
// (DBENTRYP_I)
static HB_ERRCODE sqlExGoToId(SQLEXAREA *thiswa, PHB_ITEM pItem)
{
  thiswa->firstinteract = false;
  thiswa->wasdel = false;
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExGoTop(SQLEXAREA *thiswa)
{
  thiswa->lpdbPendingRel = nullptr;
  thiswa->firstinteract = false;
  thiswa->wasdel = false;
  ...
  if (thiswa->lBofAt) {
    ...
    if (thiswa->bReverseIndex != s_bOldReverseIndex) {
      thiswa->recordListDirection = SR_LIST_FORWARD;
      SR_getOrderByExpression(thiswa, false);
      getWhereExpression(thiswa, SR_LIST_FROM_TOP);
      SR_setResultSetLimit(thiswa, RECORD_LIST_SIZE / 10);
      createRecodListQuery(thiswa);

      if (getRecordList(thiswa, RECORD_LIST_SIZE / 10) == HB_FAILURE) {
        SQLRDD::odbcErrorDiagRTE(thiswa->hStmt, "dbGoTop", thiswa->sSql, SQL_ERROR, __LINE__,
                                 __FILE__);
        SQLRDD::commonError(reinterpret_cast<AREAP>(thiswa), EG_ARG, ESQLRDD_READ,
                            thiswa->sTable.c_str());
        return HB_FAILURE;
...
// (DBENTRYP_BIB)
static HB_ERRCODE sqlExSeek(SQLEXAREA *thiswa, HB_BOOL bSoftSeek, PHB_ITEM pKey,
                            HB_BOOL bFindLast)
{
  thiswa->lpdbPendingRel = nullptr;
  thiswa->firstinteract = false;
  ...
  if (thiswa->hOrdCurrent == 0) {
    SQLRDD::commonError(reinterpret_cast<AREAP>(thiswa), EG_NOORDER, EDBF_NOTINDEXED,
                        thiswa->sTable.c_str());
    return HB_FAILURE;
  }
  ...
  if (!thiswa->CurrRecord) {
    SR_SetCurrRecordStructure(thiswa);
  }
  ...
  if (SR_CreateSeekStmt(thiswa,
                        queryLevel)) {   // Create and prepare new SEEK statement, if needed
    SR_BindSeekStmt(thiswa, queryLevel); // Bind parameters to IndexBind structure
  }
...
// (DBENTRYP_L)
static HB_ERRCODE sqlExSkip(SQLEXAREA *thiswa, HB_LONG lToSkip)
{
  HB_LONG lSkip;
  ...
  thiswa->lCurrentRecord = SR_GetCurrentRecordNum(thiswa);
...
// (DBENTRYP_L)
static HB_ERRCODE sqlExSkipFilter(SQLEXAREA *thiswa, HB_LONG lUpDown)
{
  ...
  while (!thiswa->area.fBof && !thiswa->area.fEof) {
    ...
    if (thiswa->area.dbfi.itmCobExpr) {
      if (SELF_EVALBLOCK(reinterpret_cast<AREAP>(thiswa), thiswa->area.dbfi.itmCobExpr) !=
          HB_SUCCESS) {
        return HB_FAILURE;
      }
...
// (DBENTRYP_L)
static HB_ERRCODE sqlExSkipRaw(SQLEXAREA *thiswa, HB_LONG lToSkip)
{
  HB_ERRCODE res;
...
// (DBENTRYP_B)
static HB_ERRCODE sqlExAppend(SQLEXAREA *thiswa, HB_BOOL value)
{
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExDeleteRec(SQLEXAREA *thiswa)
{
  HB_BOOL isDeleted;
  SQLRETURN res;
...
// (DBENTRYP_BP)
static HB_ERRCODE sqlExDeleted(SQLEXAREA *thiswa, HB_BOOL *isDeleted)
{
  if (thiswa->firstinteract) {
    SELF_GOTOP(reinterpret_cast<AREAP>(thiswa));
    thiswa->firstinteract = false;
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExFlush(SQLEXAREA *thiswa)
{
  return SELF_GOCOLD(reinterpret_cast<AREAP>(thiswa));
}
...
// (DBENTRYP_SI)
static HB_ERRCODE sqlExGetValue(SQLEXAREA *thiswa, HB_USHORT fieldNum, PHB_ITEM value)
{
  PHB_ITEM itemTemp3;
  HB_SIZE ulPos;
  ...
  if (HB_IS_NIL(itemTemp)) {
    getMissingColumn(thiswa,
                     hb_arrayGetItemPtr(thiswa->aBuffer, thiswa->uiBufferIndex[fieldNum - 1]),
                     static_cast<HB_LONG>(thiswa->uiBufferIndex[fieldNum - 1]));
...
#ifdef __XHARBOUR__
    itemTemp3 = hb_arrayClone(itemTemp, nullptr);
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExGoCold(SQLEXAREA *thiswa)
{
  if (thiswa->bufferHot) { // && (!(thiswa->ulhDeleted > 0 ? true :
                           // thiswa->deletedList[thiswa->recordListPos] == '
                           // ') )
    if (thiswa->bHistoric) {
    ...
    if (thiswa->bIsInsert) {
      if (!thiswa->hStmtInsert) {    // Check if we have the INSERT statement prepared
        SR_CreateInsertStmt(thiswa); // Create also column binding structures

        if (SR_PrepareInsertStmt(thiswa) == HB_FAILURE) {
          return HB_FAILURE;
        }

        if (SR_BindInsertColumns(thiswa) == HB_FAILURE) {
          return HB_FAILURE;
        }
      } else {
      ...
      if (SR_FeedRecordCols(thiswa, false) ==
          HB_FAILURE) { // Stmt created and prepared, only need to push data
        return HB_FAILURE;
      }

      if (SR_ExecuteInsertStmt(thiswa) == HB_FAILURE) {
        return HB_FAILURE;
      }
      ...
      if ((!thiswa->hStmtUpdate) ||
          memcmp((const void *)(thiswa->editMask), (const void *)(thiswa->updatedMask),
                 MAX_FIELDS) != 0) {
        if (SR_CreateUpdateStmt(thiswa) == HB_FAILURE) {
          return HB_FAILURE;
        }
      } else {
      ...
      if (SR_ExecuteUpdateStmt(thiswa) == HB_FAILURE) {
        return HB_FAILURE;
      }
    }
...
// (DBENTRYP_SI)
static HB_ERRCODE sqlExPutValue(SQLEXAREA *thiswa, HB_USHORT fieldNum, PHB_ITEM value)
{
  if (thiswa->firstinteract) {
    SELF_GOTOP(reinterpret_cast<AREAP>(thiswa));
    thiswa->firstinteract = false;
  ...
  if ((HB_IS_NUMBER(pDest) && HB_IS_NUMBER(value)) ||
    ...
    sprintf(type_err, "data type origin: %i - data type target %i", hb_itemType(value),
            hb_itemType(pDest));
    SQLRDD::commonError(reinterpret_cast<AREAP>(thiswa), EG_DATATYPE, ESQLRDD_DATATYPE,
                        type_err);
    return HB_FAILURE;
#endif
  }
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExRecall(SQLEXAREA *thiswa)
{
  HB_BOOL isDeleted;
  SQLRETURN res;
...
// (DBENTRYP_ULP)
static HB_ERRCODE sqlExRecCount(SQLEXAREA *thiswa, HB_ULONG *recCount)
{
  if (thiswa->lpdbPendingRel) {
    SELF_FORCEREL(reinterpret_cast<AREAP>(thiswa));
  }
...
// (DBENTRYP_ULP)
static HB_ERRCODE sqlExRecNo(SQLEXAREA *thiswa, HB_ULONG *recno)
{
#ifdef SQLRDD_NWG_SPECIFIC
  if (thiswa->bIsInsert) {
    SQLRDD::commonError(reinterpret_cast<AREAP>(thiswa), EG_ARG, ESQLRDD_NOT_COMMITED_YET,
                        nullptr);
    return HB_FAILURE;
  }
#endif
  ...
  *recno = SR_GetCurrentRecordNum(thiswa);
...
// (DBENTRYP_I)
static HB_ERRCODE sqlExRecId(SQLEXAREA *thiswa, PHB_ITEM recno)
{
  if (thiswa->lpdbPendingRel) {
    SELF_FORCEREL(reinterpret_cast<AREAP>(thiswa));
  } else if (thiswa->firstinteract) {
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExClose(SQLEXAREA *thiswa)
{
  HB_ERRCODE code;
  if (SELF_GOCOLD(reinterpret_cast<AREAP>(thiswa)) == HB_FAILURE) {
    return HB_FAILURE;
...
// (DBENTRYP_VO)
static HB_ERRCODE sqlExCreate(SQLEXAREA *thiswa, LPDBOPENINFO OpenInfo)
{
  HB_ERRCODE err;
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExNewArea(SQLEXAREA *thiswa)
{
  HB_ERRCODE errCode;
  // int i;
  ...
  thiswa->lRecordToRetrieve =
      static_cast<HB_ULONG *>(hb_xgrab(s_pageReadSize * sizeof(HB_ULONG)));
  ...
  thiswa->IndexBindings =
      static_cast<INDEXBIND **>(hb_xgrab(sizeof(INDEXBIND *) * MAX_INDEXES));
...
// (DBENTRYP_VO)
static HB_ERRCODE sqlExOpen(SQLEXAREA *thiswa, LPDBOPENINFO OpenInfo)
{
  HB_ERRCODE errCode;
...
// (DBENTRYP_SP)
static HB_ERRCODE sqlExStructSize(SQLEXAREA *thiswa, HB_USHORT *StructSize)
{
...
// (DBENTRYP_VOI)
static HB_ERRCODE sqlExOrderListAdd(SQLEXAREA *thiswa, LPDBORDERINFO pOrderInfo)
{
  HB_ERRCODE err;
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExOrderListClear(SQLEXAREA *thiswa)
{
  thiswa->lBofAt = 0;
  thiswa->lEofAt = 0;
  thiswa->indexLevel = -1;
...
// (DBENTRYP_VOI)
static HB_ERRCODE sqlExOrderListFocus(SQLEXAREA *thiswa, LPDBORDERINFO pOrderInfo)
{
  HB_ERRCODE err;
  HB_LONG hOldOrder = thiswa->hOrdCurrent;
...
// (DBENTRYP_VOC)
static HB_ERRCODE sqlExOrderCreate(SQLEXAREA *thiswa, LPDBORDERCREATEINFO pOrderCreateInfo)
{
  HB_ERRCODE err;
...
// (DBENTRYP_VOI)
static HB_ERRCODE sqlExOrderDestroy(SQLEXAREA *thiswa, LPDBORDERINFO pOrderInfo)
{
  thiswa->lBofAt = 0;
  thiswa->lEofAt = 0;
  thiswa->indexLevel = -1;
...
// (DBENTRYP_SVOI)
static HB_ERRCODE sqlExOrderInfo(SQLEXAREA *thiswa, HB_USHORT uiIndex, LPDBORDERINFO pInfo)
{
  HB_LONG lIndexes;
  HB_ERRCODE uiError;
  ...
  if (lIndexes) {
    ...
    case DBOI_KEYCOUNT: {
      ...
      if (getFirstColumnAsLong(thiswa, &lValue) == HB_FAILURE) {
        SQLRDD::odbcErrorDiagRTE(thiswa->hStmt, "OrdKeyCount", thiswa->sSql, SQL_ERROR,
                                 __LINE__, __FILE__);
        uiError = HB_FAILURE;
      } else {
...
// (DBENTRYP_V)
static HB_ERRCODE sqlExClearFilter(SQLEXAREA *thiswa)
{
  thiswa->lBofAt = 0;
  thiswa->lEofAt = 0;
  thiswa->bConditionChanged1 = true;
...
// (DBENTRYP_SI)
static HB_ERRCODE sqlExScopeInfo(SQLEXAREA *thiswa, HB_USHORT nScope, PHB_ITEM pItem)
{
  thiswa->lBofAt = 0;
  thiswa->lEofAt = 0;
  thiswa->bConditionChanged1 = true;
...
// #define sqlExSetFilter                 nullptr // Must be written to update
// thiswa->bConditionChanged

// culik 2010/07/07 implemented sqlExSetFilter
// (DBENTRYP_VFI)
static HB_ERRCODE sqlExSetFilter(SQLEXAREA *thiswa, LPDBFILTERINFO pFilterInfo)
{
  HB_ERRCODE ret;
...
// (DBENTRYP_VOS)
static HB_ERRCODE sqlExSetScope(SQLEXAREA *thiswa, LPDBORDSCOPEINFO sInfo)
{
  thiswa->lBofAt = 0;
  thiswa->lEofAt = 0;
  thiswa->bConditionChanged1 = true;
...
// (DBENTRYP_VL)
static HB_ERRCODE sqlExLock(SQLEXAREA *thiswa, LPDBLOCKINFO pLockInfo)
{
  if (SELF_GOCOLD(reinterpret_cast<AREAP>(thiswa)) == HB_FAILURE) {
    return HB_FAILURE;
  }
...
// (DBENTRYP_I)
static HB_ERRCODE sqlExUnLock(SQLEXAREA *thiswa, PHB_ITEM pRecNo)
{
  if (SELF_GOCOLD(reinterpret_cast<AREAP>(thiswa)) == HB_FAILURE) {
    return HB_FAILURE;
  }
...
HB_FUNC(SQLEX_GETFUNCTABLE)
{
  ...
  hb_retni(hb_rddInherit(pTable, &sqlTable, &sqlExSuper, static_cast<const char *>("SQLRDD")));
}
...
#define HB_DATASEG_BODY                                                                        \
  HB_DATASEG_FUNC(sqlEx1__InitSymbols)                                                         \
...
{
  if (HB_ISNUM(1)) {
    s_pageReadSize = hb_itemGetNL(hb_param(1, HB_IT_NUMERIC));
  }
}
...
{
  if (HB_ISNUM(1)) {
    s_bufferPoolSize = hb_itemGetNL(hb_param(1, HB_IT_NUMERIC));
  }
}
