...
HB_FUNC_STATIC(SR_FBBEGINTRANSACTION4)
{
  GET_FB_SESSION(session, 1);
...
// SR_FBBeginTransaction(hEnv)
HB_FUNC_STATIC(SR_FBCOMMITTRANSACTION4)
{
  GET_FB_SESSION(session, 1);
...
HB_FUNC_STATIC(SR_FBROLLBACKTRANSACTION4)
{
  GET_FB_SESSION(session, 1);
...
HB_FUNC_STATIC(SR_FBEXECUTEIMMEDIATE4)
{
  GET_FB_SESSION(session, 1);
  auto command = hb_parcx(2);
  // ISC_STATUS r;
...
HB_FUNC_STATIC(SR_FBDESCRIBECOL4)
{
  GET_FB_SESSION(session, 1);
  int icol = hb_parni(2);
  int dtype, rettype, i;
...
HB_FUNC_STATIC(SR_FBNUMRESULTCOLS4)
{
  GET_FB_SESSION(session, 1);
...
// SR_FBCREATEDB4(cDatabaseName, cUserName, cPassword, nPageSize=default, cCharset=default,
// nDialect=3) -> numeric (SQL_SUCCESS/SQL_ERROR)
HB_FUNC(SR_FBCREATEDB4)
{
...
static void sr_FBFieldGet4(PHB_ITEM pField, PHB_ITEM pItem, char *bBuffer,
...
// SR_FBLINEPROCESSED4(pSession, p2, aFields, lQueryOnly, nSystemID, lTranslate, aReturn) ->
// numeric/NIL NOTES: parameter 'p2' not used parameter 'lQueryOnly' not used parameter
// 'nSystemID' not used
HB_FUNC_STATIC(SR_FBLINEPROCESSED4)
{
  GET_FB_SESSION(session, 1);
  int icol, cols;
  int dtype, i;
  ...
  if (session != nullptr) {
    ...
    for (icol = 1; icol <= cols; icol++) {
      ...
      if (lIndex == 0) {
        ...
        if ((var->sqltype & 1) && (*var->sqlind < 0)) {
          sr_FBFieldGet4(hb_arrayGetItemPtr(pFields, icol), &temp, const_cast<char *>(""), 0,
          ...
          switch (dtype) {
          case IB_SQL_TEXT: {
            sr_FBFieldGet4(hb_arrayGetItemPtr(pFields, icol), &temp,
          ...
          case IB_SQL_TYPE_DATE: {
            ...
            sr_FBFieldGet4(hb_arrayGetItemPtr(pFields, icol), &temp, static_cast<char *>(data),
