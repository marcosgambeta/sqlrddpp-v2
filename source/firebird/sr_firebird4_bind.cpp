...
// SR_FBCREATEDB4(cDatabaseName, cUserName, cPassword, nPageSize=default, cCharset=default, nDialect=3) -> numeric (SQL_SUCCESS/SQL_ERROR)
...
static void sr_FBFieldGet4(PHB_ITEM pField, PHB_ITEM pItem, char *bBuffer, const HB_SIZE lLenBuff, /*HB_BOOL bQueryOnly,*/
...
// SR_FBLINEPROCESSED4(pSession, p2, aFields, lQueryOnly, nSystemID, lTranslate, aReturn) -> numeric/NIL
// NOTES:
  ...
  if (session != nullptr) {
    ...
    for (icol = 1; icol <= cols; icol++) {
      ...
      if (lIndex == 0) {
        ...
        if ((var->sqltype & 1) && (*var->sqlind < 0)) {
          sr_FBFieldGet4(hb_arrayGetItemPtr(pFields, icol), &temp, const_cast<char *>(""), 0, /*bQueryOnly,*/ /*ulSystemID,*/ bTranslate);
          hb_arraySetForward(pRet, icol, &temp);
        } else {
          dtype = (((XSQLVAR *)var)->sqltype & ~1);
          switch (dtype) {
          case IB_SQL_TEXT: {
            sr_FBFieldGet4(hb_arrayGetItemPtr(pFields, icol), &temp, static_cast<char *>(var->sqldata), var->sqllen, /*(bQueryOnly,*/
          ...
          case IB_SQL_VARYING: {
            vary = (VARY *)var->sqldata;
            vary->vary_string[vary->vary_length] = '\0';
            sr_FBFieldGet4(hb_arrayGetItemPtr(pFields, icol), &temp, static_cast<char *>(vary->vary_string), strlen(vary->vary_string),
          ...
          case IB_SQL_TIMESTAMP: {
            ...
            sr_FBFieldGet4(hb_arrayGetItemPtr(pFields, icol), &temp, static_cast<char *>(date_s), strlen(date_s), /*bQueryOnly,*/ /*ulSystemID,*/
                        bTranslate);
            hb_arraySetForward(pRet, icol, &temp);
            break;
          }
          ...
          case IB_SQL_SHORT: {
            ...
            if (dscale < 0) {
              ...
              if (value >= 0) {
                hb_snprintf(data, sizeof(data), "%*" ISC_INT64_FORMAT "d.%0*" ISC_INT64_FORMAT "d",
                            field_width - 1 + dscale, (ISC_INT64)value / tens, -dscale, (ISC_INT64)value % tens);
                sr_FBFieldGet4(hb_arrayGetItemPtr(pFields, icol), &temp, static_cast<char *>(data), strlen(data), /*bQueryOnly,*/ /*ulSystemID,*/
                            bTranslate);
              } else if ((value / tens) != 0) {
                hb_snprintf(data, sizeof(data), "%*" ISC_INT64_FORMAT "d.%0*" ISC_INT64_FORMAT "d",
                            field_width - 1 + dscale, (ISC_INT64)(value / tens), -dscale, (ISC_INT64) - (value % tens));
          ...
          case IB_SQL_QUAD: {
            ...
            sr_FBFieldGet4(hb_arrayGetItemPtr(pFields, icol), &temp, static_cast<char *>(read_blob), blob_size, /*bQueryOnly,*/ /*ulSystemID,*/
                        bTranslate);
            hb_arraySetForward(pRet, icol, &temp);
