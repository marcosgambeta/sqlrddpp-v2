...
//         ::RunTimeErr("", "Cannot change NULL constraint to datatype: " + ::aFields[nCol, FIELD_TYPE])
      ...
      IF nRet == SQL_SUCCESS .OR. nRet == SQL_SUCCESS_WITH_INFO .OR. nRet == SQL_NO_DATA_FOUND
         ...
         SWITCH ::oSql:nSystemID
         CASE SQLRDD_RDBMS_IBMDB2
            IF ::AlterColumns({{::aFields[nCol, FIELD_NAME], ::aFields[nCol, FIELD_TYPE], ::aFields[nCol, FIELD_LEN], ::aFields[nCol, FIELD_DEC], .F.}}, .T.)
               nRet := SQL_SUCCESS
            ELSE
               nRet := SQL_ERROR
            ENDIF
         ...
         CASE SQLRDD_RDBMS_AZURE
            ...
            CASE "C"
               ...
               EXIT
            CASE "N"
               cType := "NUMERIC (" + Str(::aFields[nCol, FIELD_LEN], 4) + "," + Str(::aFields[nCol, FIELD_DEC], 3) + ")"
               EXIT
            CASE "D"
               cType := "DATETIME"
            ENDSWITCH
