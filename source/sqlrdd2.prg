...
METHOD SR_WORKAREA:GetSelectList()
   ...
   FOR i := 1 TO nLen
      IF ::aSelectList[i] == 1
         ...
         IF ::osql:nsystemID == SQLRDD_RDBMS_POSTGR .AND. ::aFields[i, SR_FIELD_DOMAIN] == SQL_LONGVARCHARXML
            cSelectList += "::varchar"
         ENDIF
...
METHOD SR_WORKAREA:ParseIndexColInfo(cSQL)
   ...
   FOR i := 1 TO nLen
      aQuot[i] := ::QuotedNull(::aLocalBuffer[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2]], .T., , , , ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2], SR_FIELD_NULLABLE])
   NEXT i
   ...
   FOR i := 12 TO nLen
      IF SubStr(cSql, i, 1) == "@"
         ...
         IF aQuot[nIndexCol] == "NULL"  // This 90% of the problem from 1% of the cases
            ...
            cType := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, nIndexCol, 2], SR_FIELD_TYPE]
            ...
            ENDIF
         ELSE
            lNull := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, nIndexCol, 2], SR_FIELD_NULLABLE]
      ...
      IF ::lCanUpd
         FOR nThisField := 1 TO ::nFields
            ...
            IF lML .OR. ;
               ...
               ENDIF
               IF (lMemo .OR. lML) .AND. (::oSql:nSystemID == SQLRDD_RDBMS_ORACLE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_ADABAS .OR. ::oSql:nSystemID == SQLRDD_RDBMS_IBMDB2) .AND. ::aFields[nThisField, SR_FIELD_DOMAIN] != SQL_FAKE_LOB  // .OR. ::oSql:nSystemID == SQLRDD_RDBMS_CACHE
...
#endif
               ELSEIF ::aFields[nthisField, SR_FIELD_DOMAIN] == SQL_VARBINARY .AND. ::osql:nsystemID == SQLRDD_RDBMS_MSSQL7
...
METHOD SR_WORKAREA:sqlSeek(uKey, lSoft, lLast)
   ...
   IF lSoft .AND. ::lISAM .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ORACLE .AND. ::aIndex[::aInfo[SR_AINFO_INDEXORD], VIRTUAL_INDEX_NAME] != NIL .AND. HB_IsChar(uKey)
      ...
      nLen := Max(Len(::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS]) - 1, 1)      // Esse -1 é para remover o NRECNO que SEMPRE faz parte do indice !
      nCons := 0
      nLenKey := Len(uKey)
      ...
      FOR i := 1 TO nLen
         ...
         nThis := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2], SR_FIELD_LEN]
         cPart := SubStr(uKey, nCons + 1, nThis)

         AAdd(::aPosition, ::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2])

         cType := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2], SR_FIELD_TYPE]
         lNull := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2], SR_FIELD_NULLABLE]
         // unnecessary, the value is not used
         //nFDec := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2], SR_FIELD_DEC]
         //HB_SYMBOL_UNUSED(nFDec)
         // unnecessary, the value is not used
         //nFLen := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2], SR_FIELD_LEN]
   ...
   ELSEIF ::lISAM .AND. ::oSql:nSystemID != SQLRDD_RDBMS_POSTGR
      ...
      IF ValType(uKey) $ "NDLT"       // One field seek, piece of cake!
         ...
         lNull := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, 1, 2], SR_FIELD_NULLABLE]
         nFDec := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, 1, 2], SR_FIELD_DEC]
         nFLen := ::aFields[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, 1, 2], SR_FIELD_LEN]
   ...
   CASE SQLRDD_RDBMS_ORACLE
      IF Len(::aIndex) > 0 .AND. ::aInfo[SR_AINFO_INDEXORD] > 0 .AND. ::aIndex[::aInfo[SR_AINFO_INDEXORD], VIRTUAL_INDEX_NAME] != NIL
         IF ::aInfo[SR_AINFO_REVERSE_INDEX]
            ...
            cSql := "SELECT /*+ INDEX( A " + IIf(nDirection != ORD_DIR_FWD, "A$", "D$") + ::aIndex[::aInfo[SR_AINFO_INDEXORD], VIRTUAL_INDEX_NAME] + ") */ " + cJoin3 + "FROM" + cJoin1 + cTemp + " AND ROWNUM <= " + Str(::nCurrentFetch+2) + " " + ;
         ...
         CASE SQLRDD_RDBMS_ADABAS
            IF ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE
               IF ::OSQL:lSqlServer2008 .AND. SR_Getsql2008newTypes()
                  ...
                  ELSE
                     cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(!Empty(SR_SetCollation()), "COLLATE " + SR_SetCollation() + " " , "")  + IF(lNotNull, " NOT NULL", "")
                  ENDIF
               ELSE
                  cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(!Empty(SR_SetCollation()), "COLLATE " + SR_SetCollation() + " " , "")  + IIf(lNotNull, " NOT NULL", "")
               ENDIF
            ELSEIF ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .AND. aCreate[i, SR_FIELD_LEN] > s_nMininumVarchar2Size - 1 // 10
               cSql += "VARCHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
            ELSE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
            ENDIF
...
METHOD SR_WORKAREA:WherePgsMajor(aQuotedCols, lPartialSeek)
   ...
   ELSE
      ...
      IF aQuotedCols == NIL
         nLen := Len(::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS])
         FOR i := 1 TO nLen
            AAdd(aQuot, ::Quoted(::aLocalBuffer[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2]], .T., , , , lNull))
         NEXT i
      ELSE
...
METHOD SR_WORKAREA:WherePgsMinor(aQuotedCols)
   ...
   ELSE
      ...
      IF aQuotedCols == NIL
         nLen := Len(::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS])
         FOR i := 1 TO nLen
            cQot := ::Quoted(::aLocalBuffer[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2]], .T., , , , lNull)
            AAdd(aQuot, cQot)
         NEXT i
      ELSE
         nLen := Len(aQuotedCols)
...
METHOD SR_WORKAREA:AlterColumns(aCreate, lDisplayErrorMessage, lBakcup)
   ...
   FOR i := 1 TO Len(aCreate)
      ...
      ELSE
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ORACLE
            IF aCreate[i, SR_FIELD_LEN] > 30
               cSql += "VARCHAR2(" + LTrim(Str(Min(aCreate[i, SR_FIELD_LEN], 4000), 9, 0)) + ")" + IIf(lNotNull, " NOT NULL", "")
            ELSE
               IF aCreate[i, SR_FIELD_LEN] > s_nMininumVarchar2Size .AND. s_nMininumVarchar2Size < 30
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C" .OR. aCreate[i, SR_FIELD_TYPE] == "M") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SQLBAS
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MSSQL6 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_CACHE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_ADABAS .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE)
            IF ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE
               IF ::OSQL:lSqlServer2008 .AND. SR_Getsql2008newTypes()
                  IF aCreate[i, SR_FIELD_LEN] > 10
                     cSql += "VARCHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(!Empty(SR_SetCollation()), "COLLATE " + SR_SetCollation() + " " , "")  + IIf(lNotNull, " NOT NULL", "")
         ...
         CASE aCreate[i, SR_FIELD_TYPE] == "C" .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MYSQL .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MARIADB)
            IF aCreate[i, SR_FIELD_LEN] > 255
               cSql += "VARCHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
            ELSE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_IBMDB2)
            IF aCreate[i, SR_FIELD_LEN] > 255
               cSql += "VARCHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
            ELSE
               cSql += "CHARACTER (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_INGRES
            cSql += "varchar(" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lPrimary, "NOT NULL PRIMARY KEY", IIf(lNotNull, " NOT NULL", ""))

         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_INFORM
            cSql += "CHARACTER (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lPrimary, "NOT NULL PRIMARY KEY", IIf(lNotNull, " NOT NULL", ""))

         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ACCESS
            IF aCreate[i, SR_FIELD_LEN] > 254
               cSql += "TEXT"
            ELSE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ")"
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SQLANY
            IF aCreate[i, SR_FIELD_LEN] > 254
               cSql += "LONG VARCHAR "
            ELSE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ")" + "  " + IIf(lNotNull, " NOT NULL", "")
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR
            IF aCreate[i, SR_FIELD_LEN] > 254
               cSql += "VARCHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ")" + IIf(!Empty(::oSql:cCharSet), " CHARACTER SET " + ::oSql:cCharSet, "") + IIf(lNotNull, " NOT NULL", "")
            ELSE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ")" + IIf(!Empty(::oSql:cCharSet), " CHARACTER SET " + ::oSql:cCharSet, "")  + IIf(lNotNull, " NOT NULL", "")
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SYBASE
            IF aCreate[i, SR_FIELD_LEN] > 30
               cSql += "VARCHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ")" + IIf(lNotNull, " NOT NULL", "")
            ELSE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ")" + IIf(lNotNull, " NOT NULL", "")
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "D") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_ORACLE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_SQLBAS .OR. ::oSql:nSystemID == SQLRDD_RDBMS_INFORM .OR. ::oSql:nSystemID == SQLRDD_RDBMS_INGRES .OR. (::oSql:nSystemID == SQLRDD_RDBMS_MYSQL .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MARIADB) .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_CACHE)
            cSql += "DATE"

         CASE (aCreate[i, SR_FIELD_TYPE] == "D") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SYBASE
            cSql += "DATETIME"

         CASE (aCreate[i, SR_FIELD_TYPE] == "D") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_IBMDB2 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_ADABAS)
            cSql += "DATE"

         CASE (aCreate[i, SR_FIELD_TYPE] == "D") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_ACCESS .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL6 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE)
            cSql += "DATETIME NULL"

         CASE (aCreate[i, SR_FIELD_TYPE] == "D") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SQLANY
            cSql += "TIMESTAMP"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MSSQL6 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_CACHE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE)
            cSql += "BIT"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_ADABAS)
            cSql += "BOOLEAN"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. ((::oSql:nSystemID == SQLRDD_RDBMS_MYSQL .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MARIADB))
            cSql += "TINYINT"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_IBMDB2 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR)
            cSql += "SMALLINT"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SYBASE
            cSql += "BIT NOT NULL"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SQLANY
            cSql += "NUMERIC (1) NULL"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ORACLE
            cSql += "SMALLINT"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_INFORM
            cSql += "BOOLEAN"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_INGRES
            cSql += "tinyint"

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ORACLE
            cSql += "CLOB"
            cLobs += IIf(Empty(cLobs), "", ",") + SR_DBQUALIFY(AllTrim(cField), ::oSql:nSystemID)

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_IBMDB2
            cSql += "CLOB (256000) " + IIf("DB2/400" $ ::oSql:cSystemName, "",  " NOT LOGGED COMPACT")

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MSSQL6 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_INFORM .OR. ::oSql:nSystemID == SQLRDD_RDBMS_CACHE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE)
            cSql += "TEXT"

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MYSQL .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MARIADB)
            cSql += s_cMySqlMemoDataType

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ADABAS
            cSql += "LONG"

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_INGRES
            cSql += "long varchar"

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ACCESS
            cSql += "TEXT NULL"

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SYBASE
            cSql += "TEXT"

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SQLANY
            cSql += "LONG VARCHAR"

         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR
            cSql += "BLOB SUB_TYPE 1" + IIf(!Empty(::oSql:cCharSet), " CHARACTER SET " + ::oSql:cCharSet, "")

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MSSQL6 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_SYBASE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE) .AND. cField == ::cRecnoName
            IF ::oSql:lUseSequences
               cSql += "NUMERIC (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") IDENTITY"
            ELSE
            cSql += "NUMERIC (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") NOT NULL UNIQUE"
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_CACHE .AND. cField == ::cRecnoName
            cSql += "NUMERIC (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") UNIQUE " + [default objectscript '##class(] + SR_GetToolsOwner() + [SequenceControler).NEXTVAL("] + ::cFileName + [")']

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MSSQL6 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_SYBASE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_CACHE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE)
            cSql += "NUMERIC (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC])) + ") " + IIf(lNotNull, " NOT NULL ", "")

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .AND. cField == ::cRecnoName
            cSql += "NUMERIC (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") default (nextval('" + ::cOwner + LimitLen(::cFileName, 3) + "_SQ')) NOT NULL UNIQUE"
         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR
            cSql += "NUMERIC (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC])) + ")  default 0 " + IIf(lNotNull, " NOT NULL ", "")

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MYSQL .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MARIADB) .AND. cField == ::cRecnoName
            cSql += "BIGINT (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") NOT NULL UNIQUE AUTO_INCREMENT "
         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MYSQL .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MARIADB)
            cSql += s_cMySqlNumericDataType + " (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC])) + ") " + IIf(lNotNull, " NOT NULL ", "")

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ORACLE .AND. cField == ::cRecnoName
            cSql += "NUMBER (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ")" +;
                 ...
                 IIf(Empty(SR_SetTblSpaceIndx()), "", " TABLESPACE " + SR_SetTblSpaceIndx()) , "") + ")"
         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ORACLE
            cSql += "NUMBER (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ")" + IIf(lNotNull, " NOT NULL", "")

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_IBMDB2 .AND. cField == ::cRecnoName
            IF ::oSql:lUseSequences
               cSql += "DECIMAL (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1, NO CACHE)"
            ELSE
               cSql += "DECIMAL (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") NOT NULL"
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ADABAS .AND. cField == ::cRecnoName
            cSql += "DECIMAL (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") NOT NULL DEFAULT SERIAL"

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_IBMDB2 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_ADABAS)
            cSql += "DECIMAL(" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ")" + IIf(lNotNull, " NOT NULL", "")

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_INGRES .AND. cField == ::cRecnoName
            cSql += "DECIMAL (" + LTrim(Str(aCreate[i, SR_FIELD_LEN])) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") NOT NULL UNIQUE "

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_INFORM .AND. cField == ::cRecnoName
            cSql += "SERIAL NOT NULL UNIQUE"

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_INFORM .OR. ::oSql:nSystemID == SQLRDD_RDBMS_INGRES)
            cSql += "DECIMAL (" + LTrim(Str(aCreate[i, SR_FIELD_LEN])) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") " + IIf(lPrimary, "NOT NULL PRIMARY KEY ", IIf(lNotNull, " NOT NULL", ""))

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SQLBAS
            IF aCreate[i, SR_FIELD_LEN] > 15
               cSql += "NUMBER" + IIf(lPrimary, " NOT NULL", " ")
            ELSE
               cSql += "DECIMAL (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ")" + IIf(lPrimary, " NOT NULL", " ")
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_SQLANY
            cSql += "NUMERIC (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_ACCESS
            cSql += "NUMERIC"

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR .AND. cField == ::cRecnoName
           cSql += "DECIMAL (" + LTrim(Str(aCreate[i, SR_FIELD_LEN])) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") NOT NULL UNIQUE "

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_FIREBR3 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR4 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR5) .AND. cField == ::cRecnoName
           cSql += "DECIMAL (" + LTrim(Str(aCreate[i, SR_FIELD_LEN])) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) + ") GENERATED BY DEFAULT AS IDENTITY  NOT NULL UNIQUE "

         CASE (aCreate[i, SR_FIELD_TYPE] == "N") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_FIREBR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR3 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR4 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR5)
            IF aCreate[i, SR_FIELD_LEN] > 18
               cSql += "DOUBLE PRECISION" + IIf(lPrimary .OR. lNotNull, " NOT NULL", " ")
            ELSE
               cSql += "DECIMAL (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + "," + LTrim(Str(aCreate[i, SR_FIELD_DEC], 9, 0)) +  ")" + IIf(lPrimary .OR. lNotNull, " NOT NULL", " ")
         ...
         // including xml data type
         // postgresql datetime
         CASE (aCreate[i, SR_FIELD_TYPE] == "T") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_POSTGR)
            IF aCreate[i, SR_FIELD_LEN] == 4
               cSql += "time  without time zone "
            ELSE
               cSql += "timestamp  without time zone "
            ENDIF
         CASE (aCreate[i, SR_FIELD_TYPE] == "T") .AND. (::osql:nSystemID == SQLRDD_RDBMS_MYSQL .OR. ::osql:nSystemID == SQLRDD_RDBMS_MARIADB)
         IF aCreate[i, SR_FIELD_LEN] == 4
             cSql += "time "
         ELSE
             cSql += "DATETIME "
         ...
         // oracle datetime
         CASE (aCreate[i, SR_FIELD_TYPE] == "T") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_ORACLE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR3 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR4 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR5)
            cSql += "TIMESTAMP "
         CASE (aCreate[i, SR_FIELD_TYPE] == "T") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7) // .AND. ::OSQL:lSqlServer2008 .AND. SR_Getsql2008newTypes()
            cSql += "DATETIME NULL "
         CASE (aCreate[i, SR_FIELD_TYPE] == "T") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MYSQL .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MARIADB)
            cSql += "DATETIME "
         CASE (aCreate[i, SR_FIELD_TYPE] == "V") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7)
             cSql += " VARBINARY(MAX) "
      ...
      IF lDataInBackup
         ...
         IF ::oSql:nSystemID == SQLRDD_RDBMS_ORACLE .AND. ::aFields[nPos_, SR_FIELD_TYPE] $ "CM"
         ...
         ELSE
            IF ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .AND. ::aFields[nPos_, SR_FIELD_TYPE] != aBack[1, 2]
   ...
   FOR i := 1 TO Len(aCreate)
      ...
      ELSE
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_MSSQL6 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_CACHE .OR. ::oSql:nSystemID == SQLRDD_RDBMS_ADABAS .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE)
            IF ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(!Empty(SR_SetCollation()), "COLLATE " + SR_SetCollation() + " " , "")  + IIf(lNotNull, " NOT NULL", "")
            ELSEIF ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .AND. aCreate[i, SR_FIELD_LEN] > 10
               cSql += "VARCHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
            ELSE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
            ENDIF
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. ( ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR3 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR4 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR5)
            IF aCreate[i, SR_FIELD_LEN] > 254
               cSql += "VARCHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ")" + IIf(!Empty(::oSql:cCharSet), " CHARACTER SET " + ::oSql:cCharSet, "") + IIf(lNotNull, " NOT NULL", "")
            ELSE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ")" + IIf(!Empty(::oSql:cCharSet), " CHARACTER SET " + ::oSql:cCharSet, "")  + IIf(lNotNull, " NOT NULL", "")
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "M") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_FIREBR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR3 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR4 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR5)
            cSql += "BLOB SUB_TYPE 1" + IIf(!Empty(::oSql:cCharSet), " CHARACTER SET " + ::oSql:cCharSet, "")
