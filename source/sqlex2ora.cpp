...
void CreateInsertStmtOra(SQLEXORAAREAP thiswa)
{
   ...
   if( thiswa->nSystemID ==  SYSTEMID_MSSQL7 ) {
      sprintf(thiswa->sSql, "%s INSERT INTO %s (%s ) OUTPUT Inserted.%s INTO @InsertedData VALUES (%s );%s",
         declare,
         thiswa->sTable,
         sFields,
         thiswa->sRecnoName,
      ...
      sprintf(thiswa->sSql, "INSERT INTO %s (%s ) VALUES (%s )%s",
         thiswa->sTable,
         sFields,
         sParams,
         ident);
      ...
      if( (!bUpdate) || (bUpdate && (thiswa->editMask[i - 1] || thiswa->specialMask[i - 1]) ) ) {
         if( i == static_cast<int>(thiswa->sqlarea.ulhDeleted) ) {
            ...
            if( SR_itemEmpty2(pFieldData) && (!InsertRecord->isNullable)) {
               ...
               if( SetBindValue2(pFieldData, InsertRecord, bUpdate ? thiswa->hStmtUpdate : thiswa->hStmtInsert) == Harbour::FAILURE ) {
                  return Harbour::FAILURE;
               }
            }
         }
