...
void CreateInsertStmtOra(SQLEXORAAREAP thiswa)
{
  ...
  for (i = 1; i <= iCols; i++)
  {
    ...
    if (i != static_cast<int>(thiswa->sqlarea.ulhRecno))
...
HB_ERRCODE PrepareInsertStmtOra(SQLEXORAAREAP thiswa)
{
  ...
  if (!OCI_Prepare(thiswa->hStmtInsert, (thiswa->sSql)))
...
HB_ERRCODE BindInsertColumnsOra(SQLEXORAAREAP thiswa)
{
  ...
  {
    if (iCol != static_cast<int>(thiswa->sqlarea.ulhRecno))
    { // RECNO is never included in INSERT column list
      iBind++;
      ...
      case SQL_C_CHAR: {
        ...
        res = OCI_BindString(thiswa->hStmtInsert, InsertRecord->szBindName, InsertRecord->asChar.value,
                             InsertRecord->ColumnSize);
        break;
      }
      case SQL_C_BINARY: {
      ...
      case SQL_C_TYPE_TIMESTAMP: {
        ...
        // InsertRecord->asTimestamp.day, InsertRecord->asTimestamp.hour, InsertRecord->asTimestamp.minute,
...
HB_ERRCODE FeedRecordColsOra(SQLEXORAAREAP thiswa, HB_BOOL bUpdate)
{
  ...
  for (i = 1; i <= iCols; i++)
  {
    if ((!bUpdate) || (bUpdate && (thiswa->editMask[i - 1] || thiswa->specialMask[i - 1])))
    {
      ...
      else if (i != static_cast<int>(thiswa->sqlarea.ulhRecno))
      { // RECNO is never included in INSERT column list
  ...
  for (i = 0; i < iCols; i++)
  {
    if (thiswa->editMask[i] || thiswa->specialMask[i])
    {
      ...
      case SQL_C_CHAR: {
        ...
        res = OCI_BindString(thiswa->hStmtUpdate, CurrRecord->szBindName, CurrRecord->asChar.value,
                             CurrRecord->ColumnSize);
        break;
      }
      case SQL_C_BINARY: {
      ...
      case SQL_C_TYPE_TIMESTAMP: {
        ...
        OCI_DateSetDateTime(CurrRecord->asDate2, CurrRecord->asTimestamp.year, CurrRecord->asTimestamp.month,
                            CurrRecord->asTimestamp.day, CurrRecord->asTimestamp.hour, CurrRecord->asTimestamp.minute,
...
HB_ERRCODE ExecuteUpdateStmtOra(SQLEXORAAREAP thiswa)
{
  ...
  if (FeedRecordColsOra(thiswa, true) == Harbour::FAILURE)
