...
#define SR_DBQUALIFY(arg) '"' + lower(arg) + '"'
...
METHOD SR_WORKAREA:GetSelectList()
   ...
   FOR i := 1 TO nLen
      IF ::aSelectList[i] == 1
         ...
         IF ::aFields[i, SR_FIELD_DOMAIN] == SQL_LONGVARCHARXML
            cSelectList += "::varchar"
         ENDIF
      ...
      IF SubStr(cSql, i, 1) == "@"
         ...
         IF aQuot[nIndexCol] == "NULL"  // This 90% of the problem from 1% of the cases
            ...
            IF cType == "N"
               ...
               cFieldName := "A." + SR_DBQUALIFY(::aNames[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, nIndexCol, 2]])
...
METHOD SR_WORKAREA:LineCount(lMsg)
   ...
   IF ::lISAM
      ...
      IF s_nLineCountResult == 0
         ::oSql:Exec("SELECT " + SR_DBQUALIFY(::cRecnoName) + " FROM " + ::cQualifiedTableName + " ORDER BY " + SR_DBQUALIFY(::cRecnoName) + " DESC LIMIT 1" + IIf(::oSql:lComments, " /* Counting Records */", ""), lMsg, .T., @aRet)
...
//         aCols := HB_ATokens(StrTran(::aIndexMgmnt[nInd, INDEXMAN_IDXKEY], Chr(34), ""), ",")
      ...
      FOR i := 1 TO Len(aCols)
         ...
         ELSE
            cSqlA += " A." + SR_DBQUALIFY(cCol) + ","
            cSqlD += " A." + SR_DBQUALIFY(cCol) + " DESC,"
         ENDIF
...
METHOD SR_WORKAREA:OrderBy(nOrder, lAscend, lRec)
   ...
   ELSE
      IF nOrder == 0 .OR. nOrder > Len(::aIndex)
         RETURN " "
      ENDIF
      RETURN StrTran(::aIndex[nOrder, IIf(lAscend, ORDER_ASCEND, ORDER_DESEND)], ", A." + SR_DBQUALIFY(::cRecnoName), "")
   ENDIF
...
METHOD SR_WORKAREA:QuotedNull(uData, trim, nLen, nDec, nTargetDB, lNull, lMemo)
   ...
   IF Empty(uData) .AND. (!cType $ "AOH") .AND. (cType $ "DCMNT" .AND. cType != "L")
      IF lNull
         RETURN "NULL"
      ELSE
         SWITCH cType
      ...
      IF ::lCanUpd
         FOR nThisField := 1 TO ::nFields
            ...
            IF lML .OR. ;
               ...
               ENDIF
               // TODO: review
               IF (lMemo .OR. lML) .AND. ::oSql:nSystemID == SQLRDD_RDBMS_IBMDB2 .AND. ::aFields[nThisField, SR_FIELD_DOMAIN] != SQL_FAKE_LOB
            ...
            IF !lFirst
               FOR nInd := 1 TO Len(::aIndexMgmnt)
                  IF !Empty(::aIndexMgmnt[nInd, INDEXMAN_COLUMNS])
                     ...
                     cRet += ", " + SR_DBQUALIFY("INDKEY_" + ::aIndexMgmnt[nInd, INDEXMAN_COLUMNS]) + " = E'" + cKey + "' "
                  ...
                  IF !Empty(::aIndexMgmnt[nInd, INDEXMAN_FOR_CODEBLOCK])
                     ...
                     cRet += ", " + SR_DBQUALIFY("INDFOR_" + SubStr(::aIndexMgmnt[nInd, INDEXMAN_FOR_EXPRESS], 2, 3)) + " = E'" + cKey + "' "
   ...
   CASE lInsert
      ...
      IF ::lCanIns
         ...
         FOR nInd := 1 TO Len(::aIndexMgmnt)
            IF !Empty(::aIndexMgmnt[nInd, INDEXMAN_COLUMNS])
               IF ::cIns == NIL
                  cRet += IIf(!lFirst, ", ", "( ") + SR_DBQUALIFY("INDKEY_" + ::aIndexMgmnt[nInd, INDEXMAN_COLUMNS])
               ENDIF
               cKey := (::cAlias)->(SR_ESCAPESTRING(Eval(::aIndexMgmnt[nInd, INDEXMAN_KEY_CODEBLOCK]), ::oSql:nSystemID))
            ...
            IF !Empty(::aIndexMgmnt[nInd, INDEXMAN_FOR_CODEBLOCK])
               IF ::cIns == NIL
                  cRet += IIf(!lFirst, ", ", "( ") + SR_DBQUALIFY("INDFOR_" + SubStr(::aIndexMgmnt[nInd, INDEXMAN_FOR_EXPRESS], 2, 3))
               ENDIF
               cKey := (::cAlias)->(Eval(::aIndexMgmnt[nInd, INDEXMAN_FOR_CODEBLOCK]))
               cVal += IIf(!lFirst, ", E'", "( E'") + cKey + "'"
               ::aLocalBuffer[::aIndexMgmnt[nInd, INDEXMAN_FOR_COLPOS]] := cKey
...
METHOD SR_WORKAREA:sqlSeek(uKey, lSoft, lLast)
   ...
   IF ::lISAM
      ...
      IF ValType(uKey) $ "NDLT"       // One field seek, piece of cake!
         ...
         cNam := "A." + SR_DBQUALIFY(::aNames[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, 1, 2]])
...
METHOD SR_WORKAREA:sqlCreate(aStruct, cFileName, cAlias, nArea)
   ...
   ::oSql:Exec("DROP TABLE " + ::cOwner + SR_DBQUALIFY(cTblName) + " CASCADE" + IIf(::oSql:lComments, " /* create table */", ""), .F.)
   ::oSql:Commit()
   ...
   IF ::oSql:Exec("SELECT * FROM " + ::cOwner + SR_DBQUALIFY(cTblName) + IIf(::oSql:lComments, " /* check dropped table */", ""), .F.) == SQL_SUCCESS
      ::oSql:Commit()
      ::oSql:Exec("DROP TABLE " + ::cOwner + SR_DBQUALIFY(cTblName) + IIf(::oSql:nSystemID == SQLRDD_RDBMS_ORACLE, " CASCADE CONSTRAINTS", "") + IIf(::oSql:lComments, " /* create table */", ""), .T.)
      ::oSql:Commit()
      IF ::oSql:Exec("SELECT * FROM " + ::cOwner + SR_DBQUALIFY(cTblName) + IIf(::oSql:lComments, " /* check dropped table */", ""), .F.) == SQL_SUCCESS
   ...
   FOR i := 1 TO Len(aCreate)
      ...
      CASE "C"
         IF aCreate[i, SR_FIELD_LEN] > s_nMininumVarchar2Size - 1 // 10
   ...
   IF lRet .AND. ::oSql:nSystemID == SQLRDD_RDBMS_MSSQL7 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_ADABAS .OR. ::oSql:nSystemID == SQLRDD_RDBMS_AZURE // Create SR_RECNO INDEX
   // Culik 18/10/2010 se o server suporta clustered index, adicionamos o mesmo na criacao
      cSql := "CREATE " + IIf(::oSql:lClustered, " CLUSTERED " , " ") + "INDEX " + LimitLen(::cFileName, 3) + "_SR ON " + ::cOwner + SR_DBQUALIFY(cTblName) + "(" + SR_DBQUALIFY(::cRecnoName) + ") " + IIf(::oSql:lComments, " /* Unique Index */", "")
...
METHOD SR_WORKAREA:sqlOrderCreate(cIndexName, cColumns, cTag, cConstraintName, cTargetTable, aTargetColumns, lEnable)
   ...
   FOR i := 1 TO Len(::aIndexMgmnt)
      ...
      IF Len(::aIndexMgmnt[i, INDEXMAN_IDXKEY]) > 4 .AND. SubStr(::aIndexMgmnt[i, INDEXMAN_IDXKEY], 4, 1) == "@"
         ...
         HB_SYMBOL_UNUSED(cPrevPhysicalVIndexName)
      ENDIF
   NEXT i
...
//       (::cAlias)->(DBGoTop())
      ...
      IF cColFor == NIL
         DO WHILE !(::cAlias)->(Eof())
            ::oSql:Exec(::cUpd + cColIndx + " = E'" + SR_ESCAPESTRING(SR_Val2Char(Eval(bIndexKey)) + Str(RecNo(), 15), ::oSql:nSystemID) + "' WHERE " + SR_DBQUALIFY(::cRecnoName) + " = " + Str((::cAlias)->(RecNo())))
            (::cAlias)->(dbSkip())
         ENDDO
      ELSE
         DO WHILE !(::cAlias)->(Eof())
   ...
   IF Len(::aIndex) > 0 .AND. ::aInfo[SR_AINFO_INDEXORD] > 0
      ...
      IF nType == TOP_BOTTOM_SCOPE .OR. (::aIndex[::aInfo[SR_AINFO_INDEXORD], TOP_SCOPE] != NIL .AND. ::aIndex[::aInfo[SR_AINFO_INDEXORD], BOTTOM_SCOPE] != NIL .AND.;
         ...
         ELSEIF HB_IsChar(uKey)
            ...
            FOR i := 1 TO nLen
               cQot := ::aQuoted[i]
               cNam := "A." + SR_DBQUALIFY(::aNames[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2]])
...
METHOD SR_WORKAREA:WhereMajor()
   ...
   FOR i := 1 TO nLen
      c1 += IIf(!Empty(c1), " AND ", "") + "A." + SR_DBQUALIFY(::aNames[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2]]) + " @3" + Str(i - 1, 1)
   NEXT i
...
METHOD SR_WORKAREA:WhereMinor()
   ...
   FOR i := 1 TO nLen
      c1 += IIf(!Empty(c1), " AND ", "") + "A." + SR_DBQUALIFY(::aNames[::aIndex[::aInfo[SR_AINFO_INDEXORD], INDEX_FIELDS, i, 2]]) + " @6" + Str(i - 1, 1)
   NEXT i
      ...
      IF At(Chr(34) + cColumn + Chr(34), aIndexes[i, 3]) > 0 .OR. At("+" + cColumn + "+", "+" + AllTrim(aIndexes[i, 3]) + "+") > 0 .OR. ;
         ...
         IF Len(aIndexes) >= i
            ...
            ELSE
               cPhysicalVIndexName := NIL
            ENDIF
            HB_SYMBOL_UNUSED(cPhysicalVIndexName)
...
METHOD SR_WORKAREA:AlterColumns(aCreate, lDisplayErrorMessage, lBakcup)
   ...
   FOR i := 1 TO Len(aCreate)
      ...
      IF !(lCurrentIsMultLang .AND. aCreate[i, SR_FIELD_TYPE] $ "MC")
         ...
         cSql += " ADD " + "COLUMN " + SR_DBQUALIFY(AllTrim(cField))
         cSql += " "
      ...
      ELSE
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C")
            IF aCreate[i, SR_FIELD_LEN] > s_nMininumVarchar2Size -1 //10
               cSql += "VARCHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
            ELSE
               cSql += "CHAR (" + LTrim(Str(aCreate[i, SR_FIELD_LEN], 9, 0)) + ") " + IIf(lNotNull, " NOT NULL", "")
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "D")
            cSql += "DATE"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L")
            cSql += "BOOLEAN"
         ...
         //ELSE
            IF ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .AND. ::aFields[nPos_, SR_FIELD_TYPE] != aBack[1, 2]
               IF ::aFields[nPos_, SR_FIELD_TYPE] == "N" .AND. aBack[1, 2] == "C"
                  ::oSql:Exec("UPDATE " + ::cQualifiedTableName + " SET " + SR_DBQUALIFY(::aFields[nPos_, SR_FIELD_NAME]) + " = BACKUP_::text::numeric::integer", lDisplayErrorMessage)
               //ELSEif ::aFields[nPos_, SR_FIELD_TYPE] == "C" .AND. aBack[1, 2] == "N"
                  //::oSql:Exec("UPDATE " + ::cQualifiedTableName + " SET " + SR_DBQUALIFY(::aFields[nPos_, SR_FIELD_NAME]) + " = BACKUP_::numeric::integer::text", lDisplayErrorMessage)
               ELSE
...
METHOD SR_WORKAREA:AlterColumnsDirect(aCreate, lDisplayErrorMessage, lBakcup, aRemove)
   ...
   FOR i := 1 TO Len(aCreate)
      ...
      IF !(lCurrentIsMultLang .AND. aCreate[i, SR_FIELD_TYPE] $ "MC")
         ...
         cSql := "ALTER TABLE " + ::cQualifiedTableName
         cSql += " ALTER " + SR_DBQUALIFY(AllTrim(cField)) + " TYPE "
         cSql += " "
      ...
      ELSE
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "C") .AND. ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "N")
            ...
            cSql2 := cSql2 + " ALTER " + SR_DBQUALIFY(AllTrim(cField)) + " SET DEFAULT 0"

            IF lNotNull
               cSql3 := "ALTER TABLE " + ::cQualifiedTableName
               cSql3 := cSql3 + " ALTER " + SR_DBQUALIFY(AllTrim(cField)) + " SET "
...
METHOD SR_WORKAREA:CreateConstraint(cSourceTable, aSourceColumns, cTargetTable, aTargetColumns, cConstraintName)
   ...
   IF Len(aRet) == 0
      ...
      IF lPk
         cSql := "ALTER TABLE " + ::cOwner + StrTran(SR_DBQUALIFY(cSourceTable), Chr(34), "") + " ADD CONSTRAINT " + cConstraintName + " PRIMARY KEY (" + cTargetColumns + ")"
      ELSE
         cSql := "ALTER TABLE " + ::cOwner + StrTran(SR_DBQUALIFY(cSourceTable), Chr(34), "") + " ADD CONSTRAINT " + cConstraintName + " FOREIGN KEY (" + cSourceColumns + ") REFERENCES " + ::cOwner + StrTran(SR_DBQUALIFY(cTargetTable), Chr(34), "") + " (" + cTargetColumns + ")"
      ENDIF
