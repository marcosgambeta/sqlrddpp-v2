      ...
      IF ::lCanUpd
         FOR nThisField := 1 TO ::nFields
            ...
            IF lML .OR. ;
               ...
               ENDIF
               // TODO: review
               IF (lMemo .OR. lML) .AND. ::oSql:nSystemID == SQLRDD_RDBMS_IBMDB2 .AND. ::aFields[nThisField, 6] != SQL_FAKE_LOB
   ...
   FOR i := 1 TO Len(aCreate)
      ...
      IF (nPos_ := AScan(::aNames, {|x|AllTrim(Upper(x)) == cField})) > 0
         ...
         IF lBakcup .AND. (!lCurrentIsMultLang)
            ...
            ::oSql:Exec("UPDATE " + ::cQualifiedTableName + " SET BACKUP_ = " + SR_DBQUALIFY(::aFields[nPos_, 1]), lDisplayErrorMessage)
            ::oSql:Commit()
            lDataInBackup := .T.
         ENDIF
      ELSE
      ...
      IF lDataInBackup
         ...
         //   ::oSql:Exec("UPDATE " + ::cQualifiedTableName + " SET " + SR_DBQUALIFY(::aFields[nPos_, 1]) + " = RTRIM( BACKUP_ )", lDisplayErrorMessage)
         //ELSE
            IF ::oSql:nSystemID == SQLRDD_RDBMS_POSTGR .AND. ::aFields[nPos_, 2] != aBack[1, 2]
               IF ::aFields[nPos_, 2] == "N" .AND. aBack[1, 2] == "C"
                  ::oSql:Exec("UPDATE " + ::cQualifiedTableName + " SET " + SR_DBQUALIFY(::aFields[nPos_, 1]) + " = BACKUP_::text::numeric::integer", lDisplayErrorMessage)
               //ELSEif ::aFields[nPos_, 2] == "C" .AND. aBack[1, 2] == "N"
                  //::oSql:Exec("UPDATE " + ::cQualifiedTableName + " SET " + SR_DBQUALIFY(::aFields[nPos_, 1]) + " = BACKUP_::numeric::integer::text", lDisplayErrorMessage)
               ELSE
                  ::oSql:Exec("UPDATE " + ::cQualifiedTableName + " SET " + SR_DBQUALIFY(::aFields[nPos_, 1]) + " = BACKUP_", lDisplayErrorMessage)
               ENDIF
            ELSE
               ::oSql:Exec("UPDATE " + ::cQualifiedTableName + " SET " + SR_DBQUALIFY(::aFields[nPos_, 1]) + " = BACKUP_", lDisplayErrorMessage)
            ENDIF
