...
//   DATA cTempDir         AS CHARACTER   INIT ""
   ...
   //DATA lClustered AS LOGICAL INIT .F. READONLY (duplicated)
   //culik 30/12/2011 adicionado para indicar se e  sqlserver versao 2008 ou superior
   //DATA lSqlServer2008 AS LOGICAL INIT .F. (duplicated)
...
METHOD SR_CONNECTION:Connect(cDSN, cUser, cPassword, nVersion, cOwner, nSizeMaxBuff, lTrace, cConnect, nPrefetch, cTargetDB, nSelMeth, nEmptyMode, nDateMode, lCounter, lAutoCommit, nTimeout)
   ...
   LOCAL hEnv := NIL
   LOCAL hDbc := NIL
   //LOCAL cVersion := "" (not used)
   //LOCAL cSystemVers := "" (not used)
   LOCAL cBuff := ""
   LOCAL aCon
   LOCAL aItem
   LOCAL aToken
