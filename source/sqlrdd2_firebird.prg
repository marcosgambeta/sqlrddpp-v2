...
METHOD SR_WORKAREA:sqlCreate(aStruct, cFileName, cAlias, nArea)
   ...
   ::oSql:Exec("DROP TABLE " + ::cOwner + SR_DBQUALIFY(cTblName, ::oSql:nSystemID) + "" + IIf(::oSql:lComments, " /* create table */", ""), .F.)
   ::oSql:Commit()
   ...
   IF ::oSql:Exec("SELECT * FROM " + ::cOwner + SR_DBQUALIFY(cTblName, ::oSql:nSystemID) + IIf(::oSql:lComments, " /* check dropped table */", ""), .F.) == SQL_SUCCESS
      ::oSql:Commit()
      ::oSql:Exec("DROP TABLE " + ::cOwner + SR_DBQUALIFY(cTblName, ::oSql:nSystemID) + "" + IIf(::oSql:lComments, " /* create table */", ""), .T.)
      ::oSql:Commit()
      IF ::oSql:Exec("SELECT * FROM " + ::cOwner + SR_DBQUALIFY(cTblName, ::oSql:nSystemID) + IIf(::oSql:lComments, " /* check dropped table */", ""), .F.) == SQL_SUCCESS
...
METHOD SR_WORKAREA:AlterColumns(aCreate, lDisplayErrorMessage, lBakcup)
   ...
   FOR i := 1 TO Len(aCreate)
      ...
      IF !(lCurrentIsMultLang .AND. aCreate[i, FIELD_TYPE] $ "MC")
         ...
         cSql += " ADD " + "" + SR_DBQUALIFY(AllTrim(cField), ::oSql:nSystemID)
         cSql += " "
