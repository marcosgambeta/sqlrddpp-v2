...
METHOD SR_WORKAREA:LoadRegisteredTags()
   ...
   LOCAL lCDXCompat := .F.
   LOCAL aRet
   //LOCAL aThisIndex (variable not used)
   //LOCAL aCols (variable not used)
...
METHOD SR_WORKAREA:QuotedNull(uData, trim, nLen, nDec, nTargetDB, lNull, lMemo)
   ...
   IF Empty(uData) .AND. (!cType $ "AOH") .AND. (cType != "L")
      IF lNull
         RETURN "NULL"
      ELSE
         SWITCH cType
...
METHOD SR_WORKAREA:WriteBuffer(lInsert, aBuffer)
   ...
   IF ::lHistoric .AND. !lMustUPD
      ...
      IF !lInsert
         IF ::lVers
            lInsert := .T.
            IF (!::oSql:lUseSequences .OR. !::lUseSequences) .OR. (::oSql:nSystemID == SQLRDD_RDBMS_FIREBR)
      ...
      IF ::lCanUpd
         FOR nThisField := 1 TO ::nFields
            ...
            IF lML .OR. ;
               ...
               ENDIF
               // TODO: review
               IF (lMemo .OR. lML) .AND. (::oSql:nSystemID == SQLRDD_RDBMS_ORACLE) .AND. ::aFields[nThisField, SR_FIELD_DOMAIN] != SQL_FAKE_LOB
            ...
            IF !lFirst
               FOR nInd := 1 TO Len(::aIndexMgmnt)
                  IF !Empty(::aIndexMgmnt[nInd, INDEXMAN_COLUMNS])
                     ...
                     cRet += ", " + SR_DBQUALIFY("INDKEY_" + ::aIndexMgmnt[nInd, INDEXMAN_COLUMNS]) + " = '" + cKey + "' "
                  ...
                  IF !Empty(::aIndexMgmnt[nInd, INDEXMAN_FOR_CODEBLOCK])
                     ...
                     cRet += ", " + SR_DBQUALIFY("INDFOR_" + SubStr(::aIndexMgmnt[nInd, INDEXMAN_FOR_EXPRESS], 2, 3)) + " = '" + cKey + "' "
            ...
            IF i == ::hnRecno
               ...
               ELSEIF !::lQuickAppend
                  ...
                  CASE SQLRDD_RDBMS_FIREBR
                     ...
                     ENDIF
                     IF ::cIns == NIL
                        cRet += IIf(!lFirst, ", ", "( ") + SR_DBQUALIFY(::cRecnoName)
                     ENDIF
...
METHOD SR_WORKAREA:sqlCreate(aStruct, cFileName, cAlias, nArea)
   ...
   ::oSql:Exec("DROP TABLE " + ::cOwner + SR_DBQUALIFY(cTblName) + "" + IIf(::oSql:lComments, " /* create table */", ""), .F.)
   ::oSql:Commit()
   ...
   IF ::oSql:Exec("SELECT * FROM " + ::cOwner + SR_DBQUALIFY(cTblName) + IIf(::oSql:lComments, " /* check dropped table */", ""), .F.) == SQL_SUCCESS
      ::oSql:Commit()
      ::oSql:Exec("DROP TABLE " + ::cOwner + SR_DBQUALIFY(cTblName) + "" + IIf(::oSql:lComments, " /* create table */", ""), .T.)
      ::oSql:Commit()
      IF ::oSql:Exec("SELECT * FROM " + ::cOwner + SR_DBQUALIFY(cTblName) + IIf(::oSql:lComments, " /* check dropped table */", ""), .F.) == SQL_SUCCESS
   ...
   CASE SQLRDD_RDBMS_FIREBR
      IF nType < 3
         IF ::oSql:Exec("SELECT * FROM " + ::cQualifiedTableName + ;
            IIf(nType < 3, " WHERE " + SR_DBQUALIFY(::cRecnoName) + " = " + ::Quoted(uRecord, , 15, 0), "") + " FOR UPDATE WITH LOCK" + ;
...
METHOD SR_WORKAREA:DropColumn(cColumn, lDisplayErrorMessage, lRemoveFromWA)
   ...
   SWITCH ::oSql:nSystemID
   CASE SQLRDD_RDBMS_FIREBR
      nRet := ::oSql:Exec("ALTER TABLE " + ::cQualifiedTableName + " DROP " + SR_DBQUALIFY(cColumn), lDisplayErrorMessage)
      ::oSql:Commit()
   ENDSWITCH
...
METHOD SR_WORKAREA:AlterColumns(aCreate, lDisplayErrorMessage, lBakcup)
   ...
   FOR i := 1 TO Len(aCreate)
      ...
      ELSE
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "D") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_FIREBR)
            cSql += "DATE"

         CASE (aCreate[i, SR_FIELD_TYPE] == "L") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_FIREBR)
            cSql += "SMALLINT"
         ...
         CASE (aCreate[i, SR_FIELD_TYPE] == "T") .AND. (::oSql:nSystemID == SQLRDD_RDBMS_FIREBR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR3 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR4 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR5)
            cSql += "TIMESTAMP "
...
METHOD SR_WORKAREA:AlterColumnsDirect(aCreate, lDisplayErrorMessage, lBakcup, aRemove)
   ...
   LOCAL lCurrentIsMultLang := .F.
   LOCAL nPos
   //LOCAL cSql2 := "" (variable not used)
   //LOCAL cSql3 := "" (variable not used)
   ...
   FOR i := 1 TO Len(aCreate)
      ...
      IF !(lCurrentIsMultLang .AND. aCreate[i, SR_FIELD_TYPE] $ "MC")
         ...
         cSql := "ALTER TABLE " + ::cQualifiedTableName
         IF ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR3 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR4 .OR. ::oSql:nSystemID == SQLRDD_RDBMS_FIREBR5
...
// STATIC FUNCTION OracleMinVersion(cString) (static function not used)
//
//    STATIC s_reEnvVar
//
//    LOCAL cMatch
