...
extern void SQLO_FieldGet(PHB_ITEM pField, PHB_ITEM pItem, int iField, HB_BOOL bQueryOnly, HB_ULONG ulSystemID, HB_BOOL bTranslate, OCI_Resultset * rs);
...
static void createRecodListQueryOra(SQLEXORAAREAP thiswa)
{
   ...
   if( thiswa->sqlarea.ulhDeleted == 0 ) {
      if( thiswa->bIsSelect ) {
         sprintf(thiswa->sSql, "SELECT %s A.%c%s%c FROM (%s) A %s %s %s",
            thiswa->sLimit1,
...
static HB_ERRCODE getMissingColumn(SQLEXORAAREAP thiswa, PHB_ITEM pFieldData, HB_LONG lFieldPosDB)
{
   ...
   if( thiswa->colStmt[lFieldPosDB - 1].pStmt == nullptr ) {
      ...
      if( thiswa->bIsSelect ) {
         sprintf(sSql, "SELECT %c%s%c FROM (%s) WHERE %c%s%c = :sz001",
            OPEN_QUALIFIER(thiswa),
            colName,
            CLOSE_QUALIFIER(thiswa),
            thiswa->sqlarea.szDataFileName,
   ...
   if( !res ) {
      ...
      // thiswa->colStmt[lFieldPosDB - 1].pStmt = nullptr;
      return Harbour::FAILURE;
   }
...
static HB_ERRCODE getPreparedRecordList(SQLEXORAAREAP thiswa, int iMax) // Returns HB_TRUE if any result found
{
  ...
  rs = OCI_GetResultset(hStmt);
   ...
   do {
      ...
      // if( CHECK_SQL_N_OK(res) ) {
      //    return Harbour::FAILURE; // Any other error means a fault in SQL statement
      // }
      ...
      if( !thiswa->sFields ) {
         ...
         for( n = 1; n <= thiswa->sqlarea.area.uiFieldCount; n++ ) {
            ...
            if( uiFlds == 0 ) {
               ...
               if( thiswa->sqlarea.ulhDeleted == 0 ) {
                  sprintf(thiswa->sFields, "A.%c%s%c, A.%c%s%c",
                     OPEN_QUALIFIER(thiswa),
                     thiswa->sRecnoName,
                     CLOSE_QUALIFIER(thiswa),
                     OPEN_QUALIFIER(thiswa),
                  ...
                  sprintf(thiswa->sFields, "A.%c%s%c, A.%c%s%c, A.%c%s%c",
                     OPEN_QUALIFIER(thiswa),
                     thiswa->sRecnoName,
                     CLOSE_QUALIFIER(thiswa),
                     OPEN_QUALIFIER(thiswa),
...
static HB_ERRCODE sqlExOraSeek(SQLEXORAAREAP thiswa, HB_BOOL bSoftSeek, PHB_ITEM pKey, HB_BOOL bFindLast)
{
   ...
   if( getPreparedSeekora(thiswa, queryLevel, &iIndex, &hStmt, &rs) == Harbour::SUCCESS ) { // Fetch line from database, read RECNO and DELETED
      // Create a line array to hold the record
      PHB_ITEM temp;
      bool bTranslate;
      int iComp;
...
static HB_ERRCODE sqlExOraSkipRaw(SQLEXORAAREAP thiswa, HB_LONG lToSkip)
{
   ...
   if( lToSkip != 0 ) {
      ...
      if( thiswa->sqlarea.hOrdCurrent > 0 ) {
         thiswa->indexColumns = hb_arrayLen(hb_arrayGetItemPtr(hb_arrayGetItemPtr(thiswa->sqlarea.aOrders, static_cast<HB_ULONG>(thiswa->sqlarea.hOrdCurrent)), INDEX_FIELDS));
      } else {
         thiswa->indexColumns = 1; // Natural order, RECNO
      }
...
static HB_ERRCODE sqlExOraGetValue(SQLEXORAAREAP thiswa, HB_USHORT fieldNum, PHB_ITEM value)
{
   ...
   if( HB_IS_NIL(itemTemp) ) {
      getMissingColumn(thiswa, hb_arrayGetItemPtr(thiswa->sqlarea.aBuffer, thiswa->sqlarea.uiBufferIndex[fieldNum - 1]), static_cast<HB_LONG>(thiswa->sqlarea.uiBufferIndex[fieldNum - 1]));
