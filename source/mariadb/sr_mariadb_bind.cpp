...
mysql_error(session->dbh));
  ...
  for (row = 0; row < rows; row++) {
    ...
    case MYSQL_LONG_TYPE: {
      hb_itemPutC(temp, "N");
      hb_arraySetForward(atemp, SR_FIELD_TYPE, temp);
      hb_arraySetForward(atemp, SR_FIELD_LEN, hb_itemPutNI(temp, HB_MIN(11,
static_cast<int>(field->length)))); hb_arraySetForward(atemp, SR_FIELD_DEC, hb_itemPutNI(temp, 0));
      hb_arraySetForward(atemp, SR_FIELD_DOMAIN, hb_itemPutNI(temp, SQL_NUMERIC));
      break;
    }
    case MYSQL_INT24_TYPE: {
      hb_itemPutC(temp, "N");
      hb_arraySetForward(atemp, SR_FIELD_TYPE, temp);
      hb_arraySetForward(atemp, SR_FIELD_LEN, hb_itemPutNI(temp, HB_MIN(8,
static_cast<int>(field->length)))); hb_arraySetForward(atemp, SR_FIELD_DEC, hb_itemPutNI(temp, 0));
      hb_arraySetForward(atemp, SR_FIELD_DOMAIN, hb_itemPutNI(temp, SQL_NUMERIC));
      break;
    }
    ...
    case MYSQL_DOUBLE_TYPE: {
      ...
      hb_arraySetForward(atemp, SR_FIELD_LEN, hb_itemPutNI(temp, static_cast<int>(field->length)));
