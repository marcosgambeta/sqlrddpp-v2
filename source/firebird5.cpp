...
#define ERRORLOGANDEXIT(session, from) { fb_log_status5(session, from); hb_retnl(SQL_ERROR); return; }
...
HB_FUNC( FBBEGINTRANSACTION5 ) // FBBeginTransaction(hEnv)
{
   auto session = static_cast<PFB_SESSION>(hb_itemGetPtr(hb_param(1, Harbour::Item::POINTER)));
...
HB_FUNC( FBCOMMITTRANSACTION5 ) // FBBeginTransaction(hEnv)
{
   auto session = static_cast<PFB_SESSION>(hb_itemGetPtr(hb_param(1, Harbour::Item::POINTER)));
...
HB_FUNC( FBROLLBACKTRANSACTION5 ) // FBRollBackTransaction(hEnv)
{
   auto session = static_cast<PFB_SESSION>(hb_itemGetPtr(hb_param(1, Harbour::Item::POINTER)));
...
HB_FUNC( FBNUMRESULTCOLS5 ) // FBNumResultCols(hEnv, @nResultSetColumnCount)
{
   auto session = static_cast<PFB_SESSION>(hb_itemGetPtr(hb_param(1, Harbour::Item::POINTER)));
   ...
   if( session ) {
      ...
      for( icol = 1; icol <= cols; icol++ ) {
         ...
         if( lIndex == 0 ) {
            ...
            if( (var->sqltype & 1) && (*var->sqlind < 0) ) {
               FBFieldGet5(hb_arrayGetItemPtr(pFields, icol), temp, const_cast<char*>(""), 0, bQueryOnly, ulSystemID, bTranslate);
               hb_arraySetForward(pRet, icol, temp);
            } else {
               dtype = ((static_cast<XSQLVAR*>(var))->sqltype & ~1);
               switch( dtype ) {
                  case IB_SQL_TEXT:
                     FBFieldGet5(hb_arrayGetItemPtr(pFields, icol), temp, static_cast<char*>(var->sqldata), var->sqllen, bQueryOnly, ulSystemID, bTranslate);
                     hb_arraySetForward(pRet, icol, temp);
                     break;
                  case IB_SQL_BOOLEAN: {
                  ...
                  case IB_SQL_VARYING:
                     vary = reinterpret_cast<VARY*>(var->sqldata);
                     vary->vary_string[vary->vary_length] = '\0';
                     FBFieldGet5(hb_arrayGetItemPtr(pFields, icol), temp, static_cast<char*>(vary->vary_string), strlen(vary->vary_string), bQueryOnly, ulSystemID, bTranslate);
                     hb_arraySetForward(pRet, icol, temp);
                     break;
                  case IB_SQL_TIMESTAMP:
                     isc_decode_timestamp(reinterpret_cast<ISC_TIMESTAMP ISC_FAR*>(var->sqldata), &times);
                     ...
                     FBFieldGet5(hb_arrayGetItemPtr(pFields, icol), temp, static_cast<char*>(date_s), strlen(date_s), bQueryOnly, ulSystemID, bTranslate);
                     hb_arraySetForward(pRet, icol, temp);
                     break;
                  case IB_SQL_TYPE_TIME:
                     isc_decode_sql_time(reinterpret_cast<ISC_TIME ISC_FAR*>(var->sqldata), &times);
                     ...
                     if( dscale < 0 ) {
                        ...
                        if( value >= 0 ) {
                           hb_snprintf(data, sizeof(data), "%*" ISC_INT64_FORMAT "d.%0*" ISC_INT64_FORMAT "d",
                                    ...
                                    -dscale,
                                    static_cast<ISC_INT64>(value) % tens);
                           FBFieldGet5(hb_arrayGetItemPtr(pFields, icol), temp, static_cast<char*>(data), strlen(data), bQueryOnly, ulSystemID, bTranslate);
                        } else if( (value / tens) != 0 ) {
                           hb_snprintf(data, sizeof(data), "%*" ISC_INT64_FORMAT "d.%0*" ISC_INT64_FORMAT "d",
                                    field_width - 1 + dscale,
                                    static_cast<ISC_INT64>(value / tens),
                  ...
                  case IB_SQL_QUAD:
                     ...
                     FBFieldGet5(hb_arrayGetItemPtr(pFields, icol), temp, static_cast<char*>(read_blob), blob_size, bQueryOnly, ulSystemID, bTranslate);
                     hb_arraySetForward(pRet, icol, temp);
