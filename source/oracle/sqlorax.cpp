...
#define GET_OCI_SESSION(session, numpar) auto session = static_cast<OCI_SESSION *>(hb_itemGetPtr(hb_param(numpar, HB_IT_POINTER)))
...
{
  GET_OCI_SESSION(Stmt, 1); // TODO: Stmt -> session
...
{
  GET_OCI_SESSION(p, 1); // TODO: p -> session
  int iPos;
