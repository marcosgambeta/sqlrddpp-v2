...
{
  auto session = static_cast<POCI_SESSION>(hb_itemGetPtr(hb_param(1, HB_IT_POINTER)));
  ...
  if (session) {
    if (lStmt) {
      ...
      while (SQLO_STILL_EXECUTING ==
             (session->status = sqlo_open2(&(session->stmt), session->dbh, hb_parcx(2), 0, nullptr))) {
        SQLO_USLEEP;
      }
    }
...
void SQLO_FieldGet(PHB_ITEM pField, PHB_ITEM pItem, char *bBuffer, HB_SIZE lLenBuff, HB_BOOL bQueryOnly,
...
HB_FUNC(SQLO_LINEPROCESSED)
{
  ...
  if (session) {
    ...
    for (i = 0; i < cols; i++) {
      ...
      if (lIndex != 0) {
        SQLO_FieldGet(hb_arrayGetItemPtr(pFields, i + 1), temp, const_cast<char *>(line[lIndex - 1]), lens[lIndex - 1],
...
HB_FUNC(ORACLEINBINDPARAM)
{
  ...
  auto Stmt = static_cast<POCI_SESSION>(hb_itemGetPtr(hb_param(1, HB_IT_POINTER)));
...
{
  auto p = static_cast<POCI_SESSION>(hb_itemGetPtr(hb_param(1, HB_IT_POINTER)));
  int iPos;
