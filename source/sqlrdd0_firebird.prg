...
FUNCTION SR_GetSyntheticIndexMinimun()
   ...
   LOCAL nRet //:= s_nSyntheticIndexMinimun (value not used)
...
FUNCTION SR_SetLocks(uLocks, oCnn, nRetries)
   ...
   LOCAL lRet := .T.
   LOCAL aLocks
   LOCAL cSql := NIL // to silence warning 'is never assigned in function'
   LOCAL cValue
   LOCAL aAdded := {}
   LOCAL nRet
   LOCAL cIns := NIL // to silence warning 'is never assigned in function'
   LOCAL cDel := NIL // to silence warning 'is never assigned in function'
