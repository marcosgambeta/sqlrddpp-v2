...
HB_FUNC( SR_ODBCGETLINES ) // (::hStmt, nLenBuff, aFields, aCache, nSystemID, lTranslate, nCurrentFetch, aInfo, nDirection, hnRecno, lFetchAll, aFetch, uRecord, nPos)
{
   ...
   PHB_ITEM pFields = hb_param(3, Harbour::Item::ARRAY);
   PHB_ITEM pCache = hb_param(4, Harbour::Item::ARRAY);
   HB_ULONG ulSystemID = hb_parnl(5);
   bool bTranslate = hb_parl(6);
   int iCurrFetch = hb_parni(7);
   PHB_ITEM pInfo = hb_param(8, Harbour::Item::ARRAY);
   HB_ULONG ulDirect = hb_parnl(9);
   HB_ULONG ulnRecno = hb_parnl(10);
   bool bFetchAll = hb_parl(11);
   PHB_ITEM pFetch = hb_param(12, Harbour::Item::ARRAY);
   PHB_ITEM pRec = hb_param(13, Harbour::Item::ANY);
...
{
#if ODBCVER >= 0x0300
   hb_retni(SQLSetStmtAttr((SQLHSTMT) hb_parptr(1), (SQLINTEGER) hb_parnl(2),
      HB_ISCHAR(3) ? (SQLPOINTER) hb_parc(3) : (SQLPOINTER) (HB_PTRUINT) hb_parnint(3),
      HB_ISCHAR(3) ? (SQLINTEGER) hb_parclen(3) : (SQLINTEGER) SQL_IS_INTEGER));
#else
   hb_retni(SQLSetStmtOption((SQLHSTMT) hb_parptr(1), (SQLINTEGER) hb_parnl(2),
      HB_ISCHAR(3) ? (SQLULEN) hb_parc(3) : (SQLULEN) hb_parnl(3)));
#endif
}
