...
// #undef HB_IS_OBJECT
// #define HB_IS_OBJECT(p)  ( ( p ) && HB_IS_OF_TYPE(p, HB_IT_OBJECT) && ( p
// )->item.asArray.value->uiClass != 0 )
...
static bool ProcessFields(SQLAREA *ThisDb);
static bool SetFields(SQLAREA *ThisDb);
...
// static PHB_ITEM loadTag(SQLAREA *thiswa, LPDBORDERINFO pInfo, HB_LONG * lorder);
HB_EXTERN_BEGIN
PHB_ITEM sr_loadTagDefault(SQLAREA *thiswa, LPDBORDERINFO pInfo, HB_LONG *lorder);
HB_EXTERN_END
...
static HB_LONG searchCacheFWD(SQLAREA *thiswa, HB_LONG lPreviousCacheStatus)
{
  HB_LONG lBegin, lEnd;
  HB_LONG lPosCache = hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE);

  if (!lPosCache) {
    return 0;
  }

  lBegin = hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NCACHEBEGIN);
  lEnd = hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NCACHEEND);
...
static HB_LONG searchCacheBWD(SQLAREA *thiswa, HB_LONG lPreviousCacheStatus)
{
  HB_LONG lBegin, lEnd;
  HB_LONG lPosCache = hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE);
...
static void readCachePageFWD(SQLAREA *thiswa)
{
  // PHB_ITEM pOrd = hb_itemNew(nullptr); (using stack instead of heap)
  HB_ITEM pOrd{};
  // PHB_ITEM pDel = hb_itemNew(nullptr); (using stack instead of heap)
  HB_ITEM pDel{};
  hb_itemPutNL(&pOrd, SR_ORD_DIR_FWD);
...
static void readCachePageBWD(SQLAREA *thiswa)
{
  // PHB_ITEM pOrd = hb_itemNew(nullptr); (using stack instead of heap)
  HB_ITEM pOrd{};
  // PHB_ITEM pDel = hb_itemNew(nullptr); (using stack instead of heap)
  HB_ITEM pDel{};
  hb_itemPutNL(&pOrd, SR_ORD_DIR_BWD);
  hb_itemPutL(&pDel, thiswa->wasdel);
  hb_objSendMessage(thiswa->oWorkArea, s_pSym_READPAGE, 2, &pOrd, &pDel);
  // hb_itemRelease(pOrd);
  // hb_itemRelease(pDel);
...
static void setCurrentFromCache(SQLAREA *thiswa, HB_LONG lPos)
{
  HB_SIZE nPos, nLen;
  // PHB_ITEM pCol; (using stack instead of heap)
  HB_ITEM pCol{};

  hb_arraySetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE, lPos);
  ...
  for (nPos = 1, nLen = hb_arrayLen(pCacheRecord); nPos <= nLen; nPos++) {
    ...
    if (nPos == thiswa->ulhRecno) {
      hb_arraySet(thiswa->aInfo, SR_AINFO_RECNO, &pCol);
    }
    if (nPos == thiswa->ulhDeleted) {
      auto deleted = hb_itemGetCPtr(&pCol);
      hb_arraySetL(thiswa->aInfo, SR_AINFO_DELETED,
                   (*deleted == 'T' || *deleted == '*') ? true : false);
    }
    hb_arraySetForward(thiswa->aBuffer, nPos, &pCol);
  }

  if (thiswa->ulhDeleted == 0) {
    hb_arraySetL(thiswa->aInfo, SR_AINFO_DELETED, false);
  }
...
static void sqlGetBufferFromCache2(SQLAREA *thiswa, HB_LONG lPos)
{
  HB_SIZE nPos, nLen;
  // PHB_ITEM pCol; (using stack instead of heap)
  HB_ITEM pCol{};
...
static void sqlGetCleanBuffer(SQLAREA *thiswa)
{
  HB_SIZE nPos, nLen;
  // PHB_ITEM pCol; (using stack instead of heap)
  HB_ITEM pCol{};
  ...
  hb_arraySetL(thiswa->aInfo, SR_AINFO_DELETED, false);
  hb_arraySetL(thiswa->aInfo, SR_AINFO_EOF, true);
  hb_arrayGet(thiswa->aInfo, SR_AINFO_RCOUNT, &pCol);
  hb_itemPutNL(&pCol, hb_itemGetNL(&pCol) + 1);
  hb_arraySetForward(thiswa->aInfo, SR_AINFO_RECNO, &pCol);
  // hb_itemRelease(pCol);
  if (!thiswa->isam) {
    hb_arraySetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE,
                  static_cast<long>(hb_arrayLen(thiswa->aCache)) + 1);
  }
}
...
// (DBENTRYP_BP)
static HB_ERRCODE sqlBof(SQLAREA *thiswa, HB_BOOL *bof)
{
  ...
  thiswa->area.fBof = hb_arrayGetL(thiswa->aInfo, SR_AINFO_BOF);
...
// (DBENTRYP_BP)
static HB_ERRCODE sqlEof(SQLAREA *thiswa, HB_BOOL *eof)
{
  if (thiswa->firstinteract) {
    SELF_GOTOP(&thiswa->area);
    thiswa->firstinteract = false;
  ...
  if (hb_arrayGetL(thiswa->aInfo, SR_AINFO_ISINSERT) && hb_arrayGetL(thiswa->aInfo, SR_AINFO_HOT)) {
    *eof = false;
  } else {
    thiswa->area.fEof = hb_arrayGetL(thiswa->aInfo, SR_AINFO_EOF);
...
// (DBENTRYP_BP)
static HB_ERRCODE sqlFound(SQLAREA *thiswa, HB_BOOL *found)
{
  if (thiswa->lpdbPendingRel) {
    SELF_FORCEREL(&thiswa->area);
  }

  thiswa->area.fFound = hb_arrayGetL(thiswa->aInfo, SR_AINFO_FOUND);
...
// (DBENTRYP_V)
static HB_ERRCODE sqlGoBottom(SQLAREA *thiswa)
{
  thiswa->lpdbPendingRel = nullptr;
  thiswa->firstinteract = false;
  thiswa->wasdel = false;

  if (hb_arrayGetL(thiswa->aInfo, SR_AINFO_HOT)) {
    hb_objSendMessage(thiswa->oWorkArea, s_pSym_WRITEBUFFER, 0);
  }

  HB_LONG leof = hb_arrayGetNL(thiswa->aInfo, SR_AINFO_EOF_AT);

  if (hb_arrayGetNL(thiswa->aInfo, SR_AINFO_EOF_AT)) {
    HB_ITEM eofat{};
...
static HB_ERRCODE sqlGoTo(SQLAREA *thiswa, HB_LONG recno)
{
...
// (DBENTRYP_I)
static HB_ERRCODE sqlGoToId(SQLAREA *thiswa, PHB_ITEM pItem)
{
...
// (DBENTRYP_V)
static HB_ERRCODE sqlGoTop(SQLAREA *thiswa)
{
  thiswa->lpdbPendingRel = nullptr;
  thiswa->firstinteract = false;
  thiswa->wasdel = false;
  ...
  HB_LONG lbof = hb_arrayGetNL(thiswa->aInfo, SR_AINFO_BOF_AT);
...
static int SR_sqlKeyCompare(AREAP thiswa, PHB_ITEM pKey, bool fExact)
{
  ...
  PHB_ITEM pTag = sr_loadTagDefault(reinterpret_cast<SQLAREA *>(thiswa), nullptr, &lorder);
  if (pTag) {
    if (reinterpret_cast<SQLAREA *>(thiswa)->firstinteract) {
...
// (DBENTRYP_BIB)
static HB_ERRCODE sqlSeek(SQLAREA *thiswa, HB_BOOL bSoftSeek, PHB_ITEM pKey, HB_BOOL bFindLast)
{
  ...
  // pItem = hb_itemPutL(nullptr, bSoftSeek); (using stack instead of heap)
  hb_itemPutL(&pItem, bSoftSeek);
  // pItem2 = hb_itemPutL(nullptr, bFindLast); (using stack instead of heap)
  ...
  if ((hb_setGetDeleted() || thiswa->area.dbfi.itmCobExpr != nullptr) && !thiswa->area.fEof) {
    ...
    if (thiswa->area.fEof) {
      ...
      hb_arraySetForward(thiswa->aInfo, SR_AINFO_FOUND, &pItem);
    } else {
      if (SR_sqlKeyCompare(&thiswa->area, pKey, false) != 0) {
...
// (DBENTRYP_L)
static HB_ERRCODE sqlSkip(SQLAREA *thiswa, HB_LONG lToSkip)
{
  HB_ERRCODE ret;
...
// (DBENTRYP_L)
static HB_ERRCODE sqlSkipFilter(SQLAREA *thiswa, HB_LONG lUpDown)
{
...
static HB_ERRCODE ConcludeSkipraw(SQLAREA *thiswa, HB_LONG lToSkip)
{
#if 0
  if (lToSkip != 0) {
    thiswa->area.fBof = hb_arrayGetL(thiswa->aInfo, SR_AINFO_BOF);
...
// (DBENTRYP_L)
static HB_ERRCODE sqlSkipRaw(SQLAREA *thiswa, HB_LONG lToSkip)
{
  HB_BOOL bEof, bBof;
  PHB_ITEM pToSkip;

  bEof = hb_arrayGetL(thiswa->aInfo, SR_AINFO_EOF);
  bBof = hb_arrayGetL(thiswa->aInfo, SR_AINFO_BOF);
  ...
  hb_arraySetNL(thiswa->aInfo, SR_AINFO_SKIPCOUNT,
                hb_arrayGetNL(thiswa->aInfo, SR_AINFO_SKIPCOUNT) + lToSkip);

  if (thiswa->isam) {
    HB_LONG lPosCache, lFound, lPreviousCacheStatus;
    HB_BOOL bCurrentDeleted;

    lPosCache = hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE);
    ...
    if (lToSkip > 0) {
      ...
      if (iTemCompEqual(hb_arrayGetItemPtr(thiswa->aInfo, SR_AINFO_EOF_AT),
                        hb_arrayGetItemPtr(thiswa->aInfo, SR_AINFO_RECNO))) {
        hb_arraySetL(thiswa->aInfo, SR_AINFO_EOF, true);
      ...
      if (bCurrentDeleted) {
        ...
        if (lFound) {
          setCurrentFromCache(thiswa, lFound);
          hb_arraySetL(thiswa->aInfo, SR_AINFO_BOF, true);
          thiswa->area.fBof = true;
          return ConcludeSkipraw(thiswa, lToSkip);
        }
        readCachePageFWD(thiswa);
        ...
        if (iTemCompEqual(hb_arrayGetItemPtr(thiswa->aInfo, SR_AINFO_BOF_AT),
                          hb_arrayGetItemPtr(thiswa->aInfo, SR_AINFO_RECNO))) {
          // Checking optimizer
          hb_arraySetL(thiswa->aInfo, SR_AINFO_BOF, true);
          thiswa->area.fBof = true;
        ...
        if (bEof) {
          if (iTemCompEqual(hb_arrayGetItemPtr(thiswa->aInfo, SR_AINFO_EOF_AT),
                            hb_arrayGetItemPtr(thiswa->aInfo, SR_AINFO_RECNO))) {
            SELF_GOTOID(&thiswa->area, hb_arrayGetItemPtr(thiswa->aInfo, SR_AINFO_EOF_AT));
          } else {
            SELF_GOBOTTOM(&thiswa->area);
          }
          return ConcludeSkipraw(thiswa, lToSkip);
    ...
    if (lToSkip == 0) {
      hb_arraySetL(thiswa->aInfo, SR_AINFO_BOF, bBof);
      hb_arraySetL(thiswa->aInfo, SR_AINFO_EOF, bEof);
      return ConcludeSkipraw(thiswa, lToSkip);
    } else if (hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE) + lToSkip > 0 &&
               hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE) + lToSkip <=
                   static_cast<HB_LONG>(hb_arrayLen(thiswa->aCache))) {
      sqlGetBufferFromCache2(thiswa, hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE) + lToSkip);
      hb_arraySetL(thiswa->aInfo, SR_AINFO_BOF, false);
      hb_arraySetL(thiswa->aInfo, SR_AINFO_EOF, false);
      hb_arraySetL(thiswa->aInfo, SR_AINFO_FOUND, false);
      ...
      thiswa->area.fEof = false;
      return ConcludeSkipraw(thiswa, lToSkip);
    } else if (hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE) + lToSkip < 1) {
      hb_arraySetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE, 1);
      ...
      thiswa->area.fEof = false;
      return ConcludeSkipraw(thiswa, lToSkip);
    } else if (hb_arrayGetNL(thiswa->aInfo, SR_AINFO_NPOSCACHE) + lToSkip >
               static_cast<HB_LONG>(hb_arrayLen(thiswa->aCache))) {
      hb_arraySetL(thiswa->aInfo, SR_AINFO_BOF, false);
      sqlGetCleanBuffer(thiswa);
    }
...
// (DBENTRYP_B)
static HB_ERRCODE sqlAppend(SQLAREA *thiswa, HB_BOOL value)
{
  PHB_ITEM pItem;
  ...
  hb_arraySet(thiswa->aInfo, SR_AINFO_HOT, pItem);
  hb_arraySet(thiswa->aInfo, SR_AINFO_ISINSERT, pItem);
...
// (DBENTRYP_I)
static HB_ERRCODE sqlCreateFields(SQLAREA *thiswa, PHB_ITEM pStruct)
{
...
// (DBENTRYP_V)
static HB_ERRCODE sqlDeleteRec(SQLAREA *thiswa)
{
  if (thiswa->firstinteract) {
    SELF_GOTOP(&thiswa->area);
    thiswa->firstinteract = false;
...
// (DBENTRYP_BP)
static HB_ERRCODE sqlDeleted(SQLAREA *thiswa, HB_BOOL *isDeleted)
{
  if (thiswa->lpdbPendingRel) {
    SELF_FORCEREL(&thiswa->area);
  } else if (thiswa->firstinteract) {
  ...
  *isDeleted = hb_arrayGetL(thiswa->aInfo, SR_AINFO_DELETED);
...
// (DBENTRYP_SP)
static HB_ERRCODE sqlFieldCount(SQLAREA *thiswa, HB_USHORT *fieldCount)
{
...
// (DBENTRYP_V)
static HB_ERRCODE sqlFlush(SQLAREA *thiswa)
{
  ...
  if (HB_IS_ARRAY(itemTemp)) {
    ...
    if (pField_->uiType == HB_FT_STRING) {
      ...
      if (thiswa->area.cdPage && thiswa->area.cdPage != cdpDest) {
        ...
        char *pszVal =
            hb_cdpnDup(hb_itemGetCPtr(itemTemp), &nLen, thiswa->area.cdPage, cdpDest);
...
// (DBENTRYP_V)
static HB_ERRCODE sqlGoCold(SQLAREA *thiswa)
{
  if (hb_arrayGetL(thiswa->aInfo, SR_AINFO_HOT) && (!hb_arrayGetL(thiswa->aInfo, SR_AINFO_DELETED))) {
...
static HB_ERRCODE sqlPutValue(SQLAREA *thiswa, HB_USHORT fieldNum, PHB_ITEM value)
{
  ...
  if ((HB_IS_NUMBER(pDest) && HB_IS_NUMBER(value)) || (HB_IS_STRING(pDest) && HB_IS_STRING(value)) ||
    ...
    hb_arraySetForward(thiswa->aInfo, SR_AINFO_HOT, pDest);
    hb_itemRelease(pDest);
    return HB_SUCCESS;
#else
    char type_err[128];
...
// (DBENTRYP_V)
static HB_ERRCODE sqlRecall(SQLAREA *thiswa)
{
  if (thiswa->lpdbPendingRel) {
    SELF_FORCEREL(&thiswa->area);
  } else if (thiswa->firstinteract) {
...
// (DBENTRYP_ULP)
static HB_ERRCODE sqlRecCount(SQLAREA *thiswa, HB_ULONG *recCount)
{
  if (thiswa->lpdbPendingRel) {
    SELF_FORCEREL(&thiswa->area);
  }
...
// (DBENTRYP_ULP)
static HB_ERRCODE sqlRecNo(SQLAREA *thiswa, HB_ULONG *recno)
{
#ifdef SQLRDD_NWG_SPECIFIC
  if (hb_arrayGetNL(thiswa->aInfo, hb_arrayGetL(thiswa->aInfo, SR_AINFO_ISINSERT))) {
    SQLRDD::commonError(&thiswa->area, EG_ARG, ESQLRDD_NOT_COMMITED_YET, nullptr);
    return HB_FAILURE;
  }
#endif
  if (thiswa->lpdbPendingRel) {
  ...
  *recno = static_cast<HB_ULONG>(hb_arrayGetNL(thiswa->aInfo, SR_AINFO_RECNO));
...
// (DBENTRYP_I)
static HB_ERRCODE sqlRecId(SQLAREA *thiswa, PHB_ITEM recno)
{
  if (thiswa->lpdbPendingRel) {
    SELF_FORCEREL(&thiswa->area);
  } else if (thiswa->firstinteract) {
  ...
  if (thiswa->initialized) {
    hb_arrayGet(thiswa->aInfo, SR_AINFO_RECNO, recno);
  } else {
...
// (DBENTRYP_S)
static HB_ERRCODE sqlSetFieldExtent(SQLAREA *thiswa, HB_USHORT uiFieldExtent)
{
...
// (DBENTRYP_V)
static HB_ERRCODE sqlClose(SQLAREA *thiswa)
{
  HB_ERRCODE uiError;
  ...
  uiError = SUPER_CLOSE(&thiswa->area);
    ...
    SQLRDD::commonError(&thiswa->area, EG_DATATYPE, ESQLRDD_DATATYPE, nullptr);
    return HB_FAILURE;
  }
#endif
...
// (DBENTRYP_VO)
static HB_ERRCODE sqlCreate(SQLAREA *thiswa, LPDBOPENINFO pCreateInfo)
{
  ...
  thiswa->szDataFileName =
      static_cast<char *>(hb_xgrab(strlen(const_cast<char *>(pCreateInfo->abName)) + 1));
  ...
  if (s_pSym_SQLCREATE) {
    hb_objSendMessage(thiswa->oWorkArea, s_pSym_SQLCREATE, 4, thiswa->aCreate, pTable, pAlias,
                      pArea);
  }
...
// (DBENTRYP_SI)
static HB_ERRCODE sqlInfo(SQLAREA *thiswa, HB_USHORT uiIndex, PHB_ITEM pItem)
{
  HB_BOOL flag = true;
...
// (DBENTRYP_V)
static HB_ERRCODE sqlNewArea(SQLAREA *thiswa)
{
...
// (DBENTRYP_VO)
static HB_ERRCODE sqlOpen(SQLAREA *thiswa, LPDBOPENINFO pOpenInfo)
{
  ...
  thiswa->szDataFileName = static_cast<char *>(hb_xgrab(strlen((char *)pOpenInfo->abName) + 1));
  ...
  hb_objSendMessage(thiswa->oWorkArea, s_pSym_SQLOPEN, 6, pTable, &pArea, &pShared, &pReadOnly,
                    pAlias, &pConnection);
...
// (DBENTRYP_SP)
static HB_ERRCODE sqlStructSize(SQLAREA *thiswa, HB_USHORT *StructSize)
{
...
// (DBENTRYP_V)
static HB_ERRCODE sqlPack(SQLAREA *thiswa)
{
...
// (DBENTRYP_V)
static HB_ERRCODE sqlZap(SQLAREA *thiswa)
{
...
// (DBENTRYP_VR)
static HB_ERRCODE sqlChildEnd(SQLAREA *thiswa, LPDBRELINFO pRelInfo)
{
  HB_ERRCODE uiError;
...
// (DBENTRYP_VR)
static HB_ERRCODE sqlChildStart(SQLAREA *thiswa, LPDBRELINFO pRelInfo)
{
...
// (DBENTRYP_VR)
static HB_ERRCODE sqlChildSync(SQLAREA *thiswa, LPDBRELINFO pRelInfo)
{
...
// (DBENTRYP_V)
static HB_ERRCODE sqlForceRel(SQLAREA *thiswa)
{
...
// (DBENTRYP_VOI)
static HB_ERRCODE sqlOrderListAdd(SQLAREA *thiswa, LPDBORDERINFO pOrderInfo)
{
...
// (DBENTRYP_V)
static HB_ERRCODE sqlOrderListClear(SQLAREA *thiswa)
{
...
// (DBENTRYP_VOI)
static HB_ERRCODE sqlOrderListFocus(SQLAREA *thiswa, LPDBORDERINFO pOrderInfo)
{
  PHB_ITEM pTag;
  HB_LONG lorder = 0;
  ...
  if (pOrderInfo->itmOrder) {
    ...
    hb_objSendMessage(thiswa->oWorkArea, s_pSym_SQLORDERLISTFOCUS, 2, pOrderInfo->itmOrder,
                      pBagName);
...
// (DBENTRYP_VOO)
static HB_ERRCODE sqlOrderCondition(SQLAREA *thiswa, LPDBORDERCONDINFO lpdbOrdCondInfo)
{
...
// (DBENTRYP_VOC)
static HB_ERRCODE sqlOrderCreate(SQLAREA *thiswa, LPDBORDERCREATEINFO pOrderInfo)
{
  ...
  if (pOrderInfo->lpdbConstraintInfo) {
    ...
    hb_objSendMessage(thiswa->oWorkArea, s_pSym_SQLORDERCREATE, 7, pBagName, pOrderInfo->abExpr,
                      pAtomBagName, pConstrName, pTarget,
                      pOrderInfo->lpdbConstraintInfo->itmRelationKey, pEnable);

    hb_itemRelease(pConstrName);
    hb_itemRelease(pTarget);
    hb_itemRelease(pEnable);
  } else {
    hb_objSendMessage(thiswa->oWorkArea, s_pSym_SQLORDERCREATE, 3, pBagName, pOrderInfo->abExpr,
                      pAtomBagName);
  }
...
// (DBENTRYP_VOI)
static HB_ERRCODE sqlOrderDestroy(SQLAREA *thiswa, LPDBORDERINFO pOrderInfo)
{
  PHB_ITEM pTag;
  HB_LONG lorder = 0;
  ...
  if (pOrderInfo->itmOrder) {
    ...
    hb_objSendMessage(thiswa->oWorkArea, s_pSym_SQLORDERDESTROY, 2, pOrderInfo->itmOrder,
                      pBagName);
    hb_itemRelease(pBagName);
...
static HB_ERRCODE sqlSetServerSideIndexScope(SQLAREA *thiswa, int nScope, PHB_ITEM scopeValue)
{
  PHB_ITEM scopetype;
  int res;
...
// (DBENTRYP_SVOI)
static HB_ERRCODE sqlOrderInfo(SQLAREA *thiswa, HB_USHORT uiIndex, LPDBORDERINFO pInfo)
{
  ...
  if (lIndexes) {
    ...
    case DBOI_NUMBER: {
      hb_itemPutNILen(pInfo->itmResult, hb_arrayGetNI(thiswa->aInfo, SR_AINFO_INDEXORD), 3);
      break;
    }
    case DBOI_BAGNAME: {
      pTag = sr_loadTagDefault(thiswa, pInfo, &lorder);
      ...
      if (pTag) {
        ...
        if (pInfo->itmNewVal && HB_IS_LOGICAL(pInfo->itmNewVal)) {
          ...
          hb_itemPutL(hb_arrayGetItemPtr(thiswa->aInfo, SR_AINFO_REVERSE_INDEX),
                      hb_itemGetL(pInfo->itmNewVal));
          hb_arraySetForward(pTag, SR_AINDEX_ORDER_SKIP_UP, hb_itemNew(nullptr));
          hb_arraySetForward(pTag, SR_AINDEX_ORDER_SKIP_DOWN, hb_itemNew(nullptr));
    ...
    case DBOI_SCOPETOP: {
      ...
      if (pTag) {
        ...
        if (pInfo->itmNewVal) {
          sqlSetServerSideIndexScope(reinterpret_cast<SQLAREA *>(thiswa), 0, pInfo->itmNewVal);
        }
        hb_itemRelease(pTag);
      } else {
    ...
    case DBOI_SCOPEBOTTOM: {
      ...
      if (pTag) {
        ...
        if (pInfo->itmNewVal) {
          sqlSetServerSideIndexScope(reinterpret_cast<SQLAREA *>(thiswa), 1, pInfo->itmNewVal);
        }
        hb_itemRelease(pTag);
      } else {
        hb_itemClear(pInfo->itmResult);
    ...
    case DBOI_SCOPETOPCLEAR: {
      pTag = sr_loadTagDefault(thiswa, pInfo, &lorder);
      if (pTag) {
        sqlSetServerSideIndexScope(reinterpret_cast<SQLAREA *>(thiswa), 0, nullptr);
      }
      break;
    }
    case DBOI_SCOPEBOTTOMCLEAR: {
      pTag = sr_loadTagDefault(thiswa, pInfo, &lorder);
      if (pTag) {
        sqlSetServerSideIndexScope(reinterpret_cast<SQLAREA *>(thiswa), 1, nullptr);
      }
      break;
    }
    case DBOI_SCOPESET: {
      ...
      if (pTag) {
        ...
        if (pInfo->itmNewVal) {
          sqlSetServerSideIndexScope(reinterpret_cast<SQLAREA *>(thiswa), TOP_BOTTOM_SCOPE,
                                     pInfo->itmNewVal);
        }
        hb_itemRelease(pTag);
      } else {
...
// (DBENTRYP_V)
static HB_ERRCODE sqlClearFilter(SQLAREA *thiswa)
{
  if (thiswa->sqlfilter) {
...
// (DBENTRYP_I)
static HB_ERRCODE sqlFilterText(SQLAREA *thiswa, PHB_ITEM pFilter)
{
  if (thiswa->sqlfilter) {
...
// (DBENTRYP_SI)
static HB_ERRCODE sqlScopeInfo(SQLAREA *thiswa, HB_USHORT nScope, PHB_ITEM pItem)
{
  HB_LONG lorder;
  PHB_ITEM pTag, pTemp;
...
// (DBENTRYP_VFI)
static HB_ERRCODE sqlSetFilter(SQLAREA *thiswa, LPDBFILTERINFO pFilterInfo)
{
  HB_ERRCODE res;
...
// (DBENTRYP_VOS)
static HB_ERRCODE sqlSetScope(SQLAREA *thiswa, LPDBORDSCOPEINFO sInfo)
{
  PHB_ITEM scopetype;
  int res;
...
// (DBENTRYP_B)
static HB_ERRCODE sqlLocate(SQLAREA *thiswa, HB_BOOL fContinue)
{
  HB_ERRCODE err;

  err = SUPER_LOCATE(&thiswa->area, fContinue);
  hb_arraySetL(thiswa->aInfo, SR_AINFO_FOUND, thiswa->area.fFound);
...
// (DBENTRYP_VL)
static HB_ERRCODE sqlLock(SQLAREA *thiswa, LPDBLOCKINFO pLockInfo)
{
  PHB_ITEM pRecord;
  ...
  if (hb_arrayGetL(thiswa->aInfo, SR_AINFO_ISINSERT) || hb_arrayGetL(thiswa->aInfo, SR_AINFO_EOF)) {
...
// (DBENTRYP_I)
static HB_ERRCODE sqlUnLock(SQLAREA *thiswa, PHB_ITEM pRecNo)
{
  if (thiswa->firstinteract) {
    SELF_GOTOP(&thiswa->area);
    thiswa->firstinteract = false;
...
static HB_ERRCODE sqlDrop(PHB_ITEM pItemTable)
{
  ...
  if (hb_stackReturnItem()) {
    ...
    SQLRDD::commonError(nullptr, EG_DATATYPE, ESQLRDD_DATATYPE, nullptr);
    return HB_FAILURE;
  }
...
static bool ProcessFields(SQLAREA *thiswa)
{
  DBFIELDINFO field;
  HB_BYTE *fieldType;
  HB_USHORT i;
...
static bool SetFields(SQLAREA *thiswa)
{
  HB_USHORT i;
  PHB_ITEM thisfield;
...
namespace SQLRDD
{
void commonError(AREAP thiswa, const HB_USHORT uiGenCode, const HB_USHORT uiSubCode,
                 const char *filename)
{
...
// SR_ITEMCMP(cItem1, cItem2, nLenToCompare) ==> 0 == identical, < 0 if cItem1 < cIten2, > 0 ==
// cItem1 > cIten2
HB_FUNC(SR_ITEMCMP)
{
  hb_retni(strncmp(hb_itemGetCPtr(hb_param(1, HB_IT_ANY)),
                   hb_itemGetCPtr(hb_param(2, HB_IT_ANY)), hb_parnl(3)));
}
...
HB_INIT_SYMBOLS_BEGIN(sqlrdd1__InitSymbols)
  {"SQLRDD", {HB_FS_PUBLIC | HB_FS_LOCAL}, {HB_FUNCNAME(SQLRDD)}, nullptr},
  {"SQLRDD_GETFUNCTABLE", {HB_FS_PUBLIC | HB_FS_LOCAL}, {HB_FUNCNAME(SQLRDD_GETFUNCTABLE)}, nullptr}
...
HB_CALL_ON_STARTUP_BEGIN(_hb_sqlrdd_rdd_init_)
  hb_vmAtInit(hb_sqlrddRddInit, nullptr);
...
HB_FUNC(SR_SETFOUND)
{
  SQLAREA *pArea = reinterpret_cast<SQLAREA *>(hb_rddGetCurrentWorkAreaPointer());

  if (pArea) {
    auto pFound = hb_param(1, HB_IT_LOGICAL);
    if (pFound) {
      pArea->area.fFound = hb_itemGetL(pFound);
      hb_arraySetForward(pArea->aInfo, SR_AINFO_FOUND, pFound);
    }
  } else {
